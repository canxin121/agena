#!/usr/bin/env python3
"""Cross-check the misc_long_tail patch's thinking modes against models.dev
reasoning_options. Flag mismatches (toggle vs effort), empty-ropts templates,
and models absent from models.dev."""
import json, os, sys
D = "/Volumes/Rc20/Projects/agena/.claude/worktrees/github-model-catalog/.catalog-work"

patch = json.load(open(os.path.join(D, "patches/misc_long_tail.json")))["models"]
...
def ropts_kind(ro):
    ...
    for r in ro:
        if r.get("type") == "effort": return "effort"
        if r.get("type") == "toggle": return "toggle"
        if r.get("type") in ("token", "budget"): return "budget"
    return "other"
...
print(f"\n=== ropts empty (template unsupported by data) — {len(unsupported)} ===")
