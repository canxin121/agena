...
编辑 `~/.agena/config.toml`，至少保留一个 provider，并设置对应凭据。例如示例文件默认启用 Anthropic：
...
详见 [后端 API](docs/backend-api.md)。

## 配置入口

默认配置路径为 `~/.agena/config.toml`，也可以通过 `--config` 或 `AGENA_CONFIG` 指定。配置层级按以下顺序合并，后者覆盖前者：
...
cargo run -p agena-cli -- \
  ...
  --set providers.anthropic.default_model=claude-sonnet-4-6 \
  config resolve
```
...
- 主配置: `~/.agena/config.toml`，可用 `AGENA_CONFIG` 或 `--config` 改写。
- 数据库: 默认 `~/.agena/agena.db`，可用 `AGENA_DATABASE_URL` 或 `AGENA_DATABASE_PATH` 改写。
- 凭据文件 fallback: 默认 `~/.agena/auth.json`，可用 `AGENA_AUTH_FILE` 改写。
