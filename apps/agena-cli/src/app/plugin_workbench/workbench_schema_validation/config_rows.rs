use super::super::{
    ...
    ConfigRowView, DiagnosticSeverity, JsonNumber, JsonValue, PathSegment, PluginConfigFilter,
    PluginConfigFocus, PluginConfigStatusKind, PluginTransportFilter, PluginWorkbenchOverlay,
...
pub(in crate::app) fn config_row_type_meta(
...
pub(in crate::app) fn build_config_row(
    plugin: &PluginWorkbenchPlugin,
    title: &str,
...
pub(in crate::app) fn value_changed_at_path(
    before: &JsonValue,
    after: &JsonValue,
    path: &ConfigPath,
) -> bool {
...
pub(in crate::app) fn override_leaf_count(value: &JsonValue) -> usize {
    match value {
        JsonValue::Null => 0,
...
pub(in crate::app) fn title_for_config_path(
    plugin: &PluginWorkbenchPlugin,
    path: &ConfigPath,
    fallback: &str,
) -> String {
...
pub(in crate::app) fn path_description(
    plugin: &PluginWorkbenchPlugin,
    path: &ConfigPath,
) -> Option<String> {
    plugin
...
pub(in crate::app) fn path_constraints(
    plugin: &PluginWorkbenchPlugin,
    path: &ConfigPath,
) -> Vec<String> {
    plugin
...
pub(in crate::app) fn pair_constraints(
    plugin: &PluginWorkbenchPlugin,
...
pub(in crate::app) fn format_bool_checkbox(value: bool) -> String {
    if value {
        "[x]".to_owned()
    } else {
        "[ ]".to_owned()
...
pub(in crate::app) fn format_value_with_brackets(path: &ConfigPath, value: &JsonValue) -> String {
    match value {
...
pub(in crate::app) fn format_default_value(path: &ConfigPath, value: &JsonValue) -> String {
    match value {
...
pub(in crate::app) fn format_nullable_value_for_cell(value: &JsonValue) -> String {
    match value {
        JsonValue::Null => "[ Not set ]".to_owned(),
...
pub(in crate::app) fn format_default_nullable_value(value: &JsonValue) -> String {
    match value {
        JsonValue::Null => "Not set".to_owned(),
        JsonValue::String(text) => clean(truncate_text(text, 28)),
...
pub(in crate::app) fn format_number_with_unit(path: &ConfigPath, number: &JsonNumber) -> String {
...
pub(in crate::app) fn compact_duration_summary(value: u64, suffix: &str, label: &str) -> String {
    if label.is_empty() {
        format!("{value}{suffix}")
    } else {
...
pub(in crate::app) fn runtime_diagnostics(
...
pub(in crate::app) fn refresh_plugin_workbench_filter(dialog: &mut PluginWorkbenchOverlay) {
...
pub(in crate::app) fn filtered_plugin_indices(
...
impl PluginTransportFilter {
    ...
    pub(in crate::app) fn matches(self, transport: &str) -> bool {
        match self {
            Self::All => true,
...
impl PluginConfigFilter {
    ...
    pub(in crate::app) fn matches(self, kind: PluginConfigStatusKind) -> bool {
        match self {
            Self::All => true,
...
pub(in crate::app) fn next_transport_filter(
    filter: PluginTransportFilter,
) -> PluginTransportFilter {
    match filter {
...
pub(in crate::app) fn next_config_filter(filter: PluginConfigFilter) -> PluginConfigFilter {
    match filter {
...
pub(in crate::app) fn next_config_focus(
    focus: PluginConfigFocus,
    compact: bool,
) -> PluginConfigFocus {
    let _ = compact;
    match focus {
...
pub(in crate::app) fn previous_config_focus(
    focus: PluginConfigFocus,
    compact: bool,
) -> PluginConfigFocus {
    let _ = compact;
