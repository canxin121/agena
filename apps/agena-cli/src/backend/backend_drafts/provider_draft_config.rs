...
impl ProviderConfigDraft {
    ...
    pub(crate) fn active_credential_issuer(&self) -> Option<CredentialIssuer> {
    ...
    pub(crate) fn active_tokens_mut(&mut self) -> Option<&mut ProviderOAuthTokensDraft> {
        self.credential_drafts
...
use crate::backend::{
    ...
    ProviderApiSubtype, ProviderAuthConfig, ProviderAuthMode, ProviderAuthOverlay,
    ...
    ProviderOverlay, ProviderSecretSourceOverlay, ProviderStudioSaveError, ProviderStudioSaveField,
    ProviderStudioSaveValidationError, anyhow, credential_issuer_label, draft_secret_source_value,
    optional_non_empty, parse_credential_issuer, parse_oauth_expires_at_ms,
    provider_credential_drafts, trimmed_owned,
};
