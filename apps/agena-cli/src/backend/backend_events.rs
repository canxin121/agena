...
pub(super) fn session_event_requires_refresh(kind: &EventKind) -> bool {
    matches!(
        kind,
...
pub(super) fn permission_event_requires_forced_refresh(kind: &EventKind) -> bool {
    matches!(
        kind,
...
use crate::backend::{EventKind, Path, PathBuf, WalkBuilder};
