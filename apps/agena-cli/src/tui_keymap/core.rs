...
pub(super) fn resolve(context: KeyContext, key: KeyEvent) -> Option<A> {
    match context {
        ...
        KeyContext::Help => match key.code {
            ...
            K::Char('?') if unmodified_or_shift(key) => Some(A::Close),
            _ => None,
        },
        ...
        KeyContext::SessionSearch => match key.code {
            K::PageUp if only_ctrl(key) => Some(A::Previous),
            K::PageDown if only_ctrl(key) => Some(A::Next),
            K::Tab if unmodified(key) => Some(A::Fill),
            K::Enter if unmodified(key) => Some(A::Open),
            _ => None,
        },
        KeyContext::Picker => match key.code {
            K::Tab if unmodified(key) => Some(A::Fill),
            K::Char('n') if only_alt(key) => Some(A::New),
            K::Char('d') if only_alt(key) => Some(A::Delete),
            K::Enter if unmodified(key) => Some(A::Accept),
            _ => None,
        },
        ...
        KeyContext::Timeline => match key.code {
            K::Enter if unmodified(key) => Some(A::Open),
            K::Char('y') if only_alt(key) => Some(A::CopyEvent),
            _ => None,
        },
        KeyContext::Confirm => match key.code {
            K::Esc | K::Enter | K::Char('y') if unmodified(key) => match key.code {
                K::Esc => Some(A::Close),
                _ => Some(A::Confirm),
            },
            K::Char('Y') if unmodified_or_shift(key) => Some(A::Confirm),
            _ => None,
        },
        KeyContext::PermissionPrompt => match key.code {
            K::Char('i') if unmodified(key) => Some(A::Details),
            K::Esc | K::Left if unmodified(key) => Some(A::Back),
            K::Up if unmodified(key) => Some(A::MoveUp),
            K::Down if unmodified(key) => Some(A::MoveDown),
            K::Enter | K::Right if unmodified(key) => Some(A::Activate),
            _ => None,
        },
        KeyContext::UserInputQuestion => match key.code {
            ...
            K::Up | K::Char('k') if unmodified(key) => Some(A::MoveUp),
            K::Down | K::Char('j') if unmodified(key) => Some(A::MoveDown),
            K::PageUp if unmodified(key) => Some(A::PreviousQuestion),
            K::PageDown if unmodified(key) => Some(A::NextQuestion),
            K::Home if unmodified(key) => Some(A::Home),
            K::End if unmodified(key) => Some(A::End),
            ...
            K::Char('c') | K::Delete | K::Backspace if unmodified(key) => Some(A::Clear),
            _ => None,
        },
        KeyContext::UserInputReview => match key.code {
            ...
            K::Home if unmodified(key) => Some(A::Home),
            K::End if unmodified(key) => Some(A::End),
            K::BackTab if unmodified_or_shift(key) => Some(A::Edit),
            K::Char('e') | K::Left | K::Char('h') if unmodified(key) => Some(A::Edit),
            K::Char('c') | K::Delete | K::Backspace if unmodified(key) => Some(A::Clear),
            _ => None,
        },
        _ => None,
