...
impl BackendManager {
    ...
    pub async fn ensure_started(&self, app: &AppHandle) -> Result<BackendStatus, String> {
        ...
        let (child, url) = match spawn_backend_service(app, &cfg).await {
            Ok(started) => started,
            Err(err) => {
...
async fn spawn_backend_service(
    ...
    let backend_cmd = match app.shell().sidecar("agena-studio") {
        Ok(cmd) => cmd,
        Err(_) => {
    ...
    if let Some(database_path) = cfg.backend.database_path.as_deref() {
        cmd = cmd.args(["--database-path", database_path]);
    }

    if let Some(database_url) = cfg.backend.database_url.as_deref() {
        cmd = cmd.args(["--database-url", database_url]);
    }
...
fn try_cleanup_stale_backend_on_port(port: u16) -> bool {
    ...
    for pid in pids {
        ...
        if let Some(image_name) = process_name_for_pid_windows(pid) {
            ...
            if !normalized.contains("agena-studio") {
                continue;
            }
        } else {
...
fn infer_backend_spawn_hint(detail: &str) -> Option<String> {
    ...
    {
        return Some(
            "Backend binary or required files are missing. Reinstall Agena Studio Desktop."
                .to_string(),
        );
    }
