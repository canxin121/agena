...
const USER_AGENT: &str = "agena-studio-desktop-updater";
...
fn resolve_service_binary_path() -> Result<PathBuf, String> {
    ...
    let mut service_path = base_dir.join("agena-studio");
...
Get-Process agena-studio-desktop -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue; \
{launch}"
    );
...
pkill -x agena-studio >/dev/null 2>&1 || true; \
pkill -x agena-studio-desktop >/dev/null 2>&1 || true; \
({launch}) >/dev/null 2>&1 &"
    );
