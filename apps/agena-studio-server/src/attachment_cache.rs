...
mod tests {
    ...
    #[tokio::test]
    async fn attachment_cache_registers_uploaded_file() {
        ...
        let cache = AttachmentCacheManager::new(studio_db.clone());
        cache
        ...
        let encoded = sqlx::query_scalar::<_, String>(
            "SELECT bytes_b64 FROM attachment_cache_blob_store LIMIT 1",
        )
        .fetch_one(studio_db.pool())
        .await
        .expect("blob row");
        assert_eq!(encoded, "Y2FjaGVkIHVwbG9hZCBieXRlcw==");
    }
}
