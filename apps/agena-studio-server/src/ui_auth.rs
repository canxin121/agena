...
mod tests {
    ...
    #[test]
    fn parse_forwarded_for_handles_ipv6_and_quotes() {
        let parsed = parse_forwarded_for("for=\"[2001:db8::1]:4711\";proto=https, for=192.0.2.4");
        assert_eq!(parsed.as_deref(), Some("2001:db8::1"));
    }

    #[test]
    fn login_attempt_key_uses_header_precedence() {
        ...
        headers.insert("forwarded", HeaderValue::from_static("for=198.51.100.8"));
        headers.insert(header::USER_AGENT, HeaderValue::from_static("Agena Tests"));
        assert_eq!(login_attempt_key(&headers), "xff:203.0.113.10");

        headers.remove("x-forwarded-for");
        assert_eq!(login_attempt_key(&headers), "xri:198.51.100.9");

        headers.remove("x-real-ip");
        assert_eq!(login_attempt_key(&headers), "fwd:198.51.100.8");

        headers.remove("forwarded");
        assert_eq!(login_attempt_key(&headers), "ua:Agena Tests");
    }

    #[test]
    fn authorization_token_requires_single_bearer_token() {
        let mut headers = HeaderMap::new();
        headers.insert(
        ...
        assert_eq!(
            get_token_from_authorization(&headers).as_deref(),
            Some("token-123")
        );

        headers.insert(
            header::AUTHORIZATION,
            HeaderValue::from_static("Bearer token-123 trailing"),
        );
        assert_eq!(get_token_from_authorization(&headers), None);
    }

    #[test]
    fn session_cookie_for_samesite_none_is_always_secure() {
        let cookie = build_session_cookie("token", false, SameSite::None);
    ...
    #[test]
    fn public_plugin_assets_are_limited_to_safe_asset_paths() {
        assert!(is_public_plugin_asset_request(
            &Method::GET,
            "/api/plugins/demo/assets/app.js"
        ));
        assert!(is_public_plugin_asset_request(
            &Method::HEAD,
            "/api/plugins/demo/assets/style.css"
        ));
        assert!(!is_public_plugin_asset_request(
            &Method::POST,
            "/api/plugins/demo/assets/app.js"
    ...
    #[test]
    fn failed_login_attempts_lock_and_can_be_cleared() {
        let inner = empty_auth();
        let attempt_key = "xff:203.0.113.10";
        let now = OffsetDateTime::now_utc();

        for _ in 0..(LOGIN_FAILURE_LIMIT - 1) {
            assert_eq!(record_failed_login_attempt(&inner, attempt_key, now), None);
        }

        let retry_after = record_failed_login_attempt(&inner, attempt_key, now)
            .expect("lockout should start at the configured limit");
    ...
    #[test]
    fn expired_lockout_is_reset_on_next_check() {
        let inner = empty_auth();
        let attempt_key = "ua:test";
        let now = OffsetDateTime::now_utc();
        inner.login_attempts.insert(
            attempt_key.to_string(),
            LoginAttemptRecord {
                window_started: now - login_failure_window_duration() - time::Duration::seconds(1),
                failures: LOGIN_FAILURE_LIMIT,
                locked_until: Some(now - time::Duration::seconds(1)),
            },
        );
        ...
        let entry = inner
            .login_attempts
            .get(attempt_key)
            .expect("entry should still exist after reset");
        assert_eq!(entry.failures, 0);
        assert_eq!(entry.locked_until, None);
    }
}
