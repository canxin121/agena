...
overlay-provider-studio-footer = Tab focus | Enter edit/select | Space toggle adapter | / search catalog | r discover | R refresh catalog | s save provider | a save adapter matches | m save selected model | Esc close
...
overlay-provider-studio-adapters = Adapter Discovery
...
overlay-provider-studio-discovery-empty = Run discovery to probe adapters and live model lists
...
flash-provider-studio-model-required = select a catalog entry or discovered model first
...
command-provider-config-summary = Open the provider workbench for auth, adapter discovery, catalog search, and model saves
