...
status-global = Alt+S 会话 | Alt+P 命令面板 | ? 帮助 | q/Ctrl+C 退出
status-sessions = 会话：Alt+S 切换器 | /sessions [查询] | /search [查询]
...
help-sessions-line-3 = /sessions 或 /search 打开切换器；参数会预填过滤条件
...
help-actions-line-1 = n 创建会话，s 或 Alt+S 打开恢复会话列表，b 打开分支历史，[ 打开父会话，] 打开子会话列表，P 打开插件检查器
...
overlay-provider-studio-footer = Tab 切换区域 | Enter 编辑/选择 | Space 切换 adapter | / 搜索 catalog | r 列出 adapter 模型 | R 刷新 catalog | s 保存 provider | a 保存 adapter 模型列表 | m 保存当前模型 | Esc 关闭
overlay-provider-studio-search-title = Catalog 搜索
overlay-provider-studio-search-prompt = 按模型 ID、显示名、来源或描述过滤 model catalog
...
flash-provider-studio-model-required = 请先选择 catalog 条目或已列出的模型
