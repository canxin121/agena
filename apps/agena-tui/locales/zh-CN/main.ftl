...
overlay-provider-studio-footer = Tab 切换区域 | Enter 编辑/选择 | Space 切换 adapter | / 搜索 catalog | r 探测 adapter | R 刷新 catalog | s 保存 provider | a 保存 adapter 匹配模型 | m 保存当前模型 | Esc 关闭
...
flash-provider-studio-model-required = 请先选择 catalog 条目或已探测模型
...
command-provider-config-summary = 打开 provider 工作台，管理 auth、adapter 探测、catalog 搜索和模型保存
