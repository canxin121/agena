...
status-composer = 输入：Enter 入队/发送 | Ctrl+Enter 立即发送 | Alt+Up/Down 历史 | Shift+Enter 换行 | / 命令 | Tab 对话
...
help-composer-line-10 = Alt+Up/Down 召回已发送提示词；Alt+P 打开命令面板，/model、/fork、/rewind 这类 slash 命令可直接在输入框执行
...
command-settings-summary = 打开可视化配置工作台，集中管理 agena.toml 字段、运行时覆盖、provider、权限规则与配置文件入口
