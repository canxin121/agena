...
fn tool_invocation_label(invocation: &ToolInvocation) -> String {
    ...
    let input_value = serde_json::Value::from(input.clone());
    let Some(input_object) = input_value.as_object() else {
        return name.clone();
    };
    ...
    for field in LABEL_FIELDS {
        if let Some(value) = input_object.get(*field).and_then(serde_json::Value::as_str)
            && !value.trim().is_empty()
        {
