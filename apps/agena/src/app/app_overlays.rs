impl App {
    ...
    pub(in crate::app) fn handle_selection_picker_key(
        &mut self,
        key: KeyEvent,
            ...
            agena_tui::command_palette::CommandPaletteEffect::Activate { key } => {
                ...
                match action {
                    ...
                    CommandPaletteCommand::Plugin(entry) => {
                        if let Some(command_name) = plugin_command_slash_name(&entry) {
    ...
    pub(in crate::app) fn handle_session_navigation_key(
        &mut self,
        key: KeyEvent,
