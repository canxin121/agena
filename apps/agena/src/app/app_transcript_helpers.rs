...
pub(in crate::app) fn message_parts_score(parts: Option<&Vec<MessagePartResource>>) -> usize {
    parts
        .map(|parts| parts.iter().map(message_part_score).sum())
        .unwrap_or(0)
}

pub(in crate::app) fn message_part_score(part: &MessagePartResource) -> usize {
    let mut score = 0;
