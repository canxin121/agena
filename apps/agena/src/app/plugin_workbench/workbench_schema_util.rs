pub(in crate::app) fn schema_property_count(schema: &JsonValue) -> usize {
...
pub(in crate::app) fn command_argument_count(
...
pub(in crate::app) fn command_schema_and_value(
    plugin: &PluginWorkbenchPlugin,
    command: &agena_plugin_host::PluginCommandDefinition,
...
pub(in crate::app) fn schema_is_map_like(root: &JsonValue, schema: &JsonValue) -> bool {
...
pub(in crate::app) fn schema_has_map_keywords(schema: &JsonValue) -> bool {
...
pub(in crate::app) fn schema_prohibits_additional_properties(schema: &JsonValue) -> bool {
...
pub(in crate::app) fn schema_property_name_schemas(schema: &JsonValue) -> Vec<JsonValue> {
...
pub(in crate::app) fn schema_prefix_item_count(schema: &JsonValue) -> usize {
    let direct = schema
        .get("prefixItems")
        .and_then(JsonValue::as_array)
        .map(Vec::len)
...
pub(in crate::app) fn schema_min_u64_constraint(schema: &JsonValue, key: &str) -> Option<u64> {
...
pub(in crate::app) fn schema_max_u64_constraint(schema: &JsonValue, key: &str) -> Option<u64> {
    let direct = schema.get(key).and_then(JsonValue::as_u64);
    let nested = schema
        .get("allOf")
        .and_then(JsonValue::as_array)
...
pub(in crate::app) fn schema_declared_property_keys(schema: &JsonValue) -> BTreeSet<String> {
    let mut keys = schema
        .get("properties")
...
pub(in crate::app) fn schema_matches_pattern_property(schema: &JsonValue, key: &str) -> bool {
    if schema
...
pub(in crate::app) fn ordered_object_keys(
...
pub(in crate::app) fn schema_required_fields(schema: &JsonValue) -> BTreeSet<String> {
...
pub(in crate::app) fn object_field_state(
...
pub(in crate::app) fn object_array_columns(
    schema: Option<&JsonValue>,
    items: &[JsonValue],
) -> Vec<String> {
    let mut keys = Vec::new();
...
pub(in crate::app) fn structured_preview(value: &JsonValue) -> String {
    match value {
...
pub(in crate::app) fn number_constraint_summary(schema: &JsonValue) -> String {
...
pub(in crate::app) fn schema_constraints(schema: &JsonValue) -> Vec<String> {
...
pub(in crate::app) fn plugin_workbench_summary(dialog: &PluginWorkbenchOverlay) -> String {
    format!(
...
pub(in crate::app) fn wrap_prefixed_text(
    text: &str,
    first_prefix: &str,
    rest_prefix: &str,
    width: usize,
...
pub(in crate::app) fn take_width_prefix(text: &str, max_width: usize) -> String {
...
pub(in crate::app) fn plugin_package_preview(value: &JsonValue) -> String {
    value
        .get("kind")
...
pub(in crate::app) fn diagnostic_severity_label(severity: DiagnosticSeverity) -> &'static str {
    match severity {
...
pub(in crate::app) fn plugin_get_json_path(
    value: &JsonValue,
    path: Option<&str>,
...
pub(in crate::app) fn plugin_config_record_value(plugin: &PluginWorkbenchPlugin) -> JsonValue {
...
pub(in crate::app) fn move_selected_config_node(dialog: &mut PluginWorkbenchOverlay, delta: isize) {
...
pub(in crate::app) fn move_detail_scroll(dialog: &mut PluginWorkbenchOverlay, delta: isize) {
...
pub(in crate::app) fn move_index(index: &mut usize, item_count: usize, delta: isize) {
    if item_count == 0 {
        *index = 0;
        return;
    }
...
pub(in crate::app) fn clean(text: impl AsRef<str>) -> String {
    text.as_ref()
        .chars()
        .map(|ch| {
            if ch.is_control() && ch != '\n' && ch != '\t' {
