...
pub(in crate::app) fn schema_contains_keyword(schema: &JsonValue, key: &str) -> bool {
    if schema
        .as_object()
...
pub(in crate::app) fn schema_bool_keyword_any(schema: &JsonValue, key: &str) -> bool {
...
pub(in crate::app) fn schema_first_string_keyword<'a>(
    schema: &'a JsonValue,
    key: &str,
) -> Option<&'a str> {
...
pub(in crate::app) fn schema_const_value(schema: &JsonValue) -> Option<JsonValue> {
...
pub(in crate::app) fn schema_enum_values(schema: &JsonValue) -> Option<Vec<JsonValue>> {
...
pub(in crate::app) fn schema_description_text(schema: &JsonValue) -> Option<String> {
    schema
        .get("description")
...
pub(in crate::app) fn schema_examples(schema: &JsonValue) -> Option<Vec<JsonValue>> {
    schema
        .get("examples")
        .and_then(JsonValue::as_array)
        .cloned()
...
pub(in crate::app) fn array_enum_variants(
    root: &JsonValue,
    schema: &JsonValue,
...
pub(in crate::app) fn schema_uses_nullable_string_editor(schema: &JsonValue) -> bool {
...
pub(in crate::app) fn format_multi_enum_value_with_selector(values: &[JsonValue]) -> String {
    if values.is_empty() {
        "[ None ▾ ]".to_owned()
    } else {
        format!(
...
pub(in crate::app) fn format_multi_enum_default_value(values: &[JsonValue]) -> String {
    if values.is_empty() {
        "None".to_owned()
    } else {
        values
