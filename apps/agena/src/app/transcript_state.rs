...
impl TranscriptState {
    ...
    pub(in crate::app) fn apply_presentation_event(
        &mut self,
...
fn append_operation_output_delta(part: &mut crate::app::MessagePartResource, delta: &str) -> bool {
    let Some(crate::app::MessagePartDetailResource::Operation(operation)) = part.content.as_mut()
    else {
        return false;
    };
...
fn append_text_operation_block(blocks: &mut Vec<crate::app::OperationBlockResource>, delta: &str) {
    match blocks.last_mut() {
        Some(crate::app::OperationBlockResource::Text { text }) => text.push_str(delta),
        _ => blocks.push(crate::app::OperationBlockResource::Text {
            text: delta.to_owned(),
        }),
    }
}
