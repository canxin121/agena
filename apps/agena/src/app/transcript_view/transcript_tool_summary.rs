...
pub(in crate::app) fn tool_status_color(status: PartExecutionStatusResource) -> Color {
    match status {
...
pub(in crate::app) fn activity_status_icon(status: PartExecutionStatusResource) -> &'static str {
    match status {
...
pub(in crate::app) fn tool_execution_status_summary(
    part: &MessagePartResource,
    label: &str,
) -> String {
    format!("{} {label}", activity_status_icon(part.status))
