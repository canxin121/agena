...
fn main() -> Result<(), AgenaAppError> {
    agena_runtime::build_app_runtime()?.block_on(async_main())
}

async fn async_main() -> Result<(), AgenaAppError> {
    match AgenaCli::parse().into_launch_mode() {
        LaunchMode::Tui(request) => run_tui(request).await,
        LaunchMode::AppServer(request) => run_app_server(request).await,
        LaunchMode::Command(cli) => run_command(cli).await,
    }
}

async fn run_tui(request: TuiLaunchRequest) -> Result<(), AgenaAppError> {
    let args = request.args;
    let launch_args = agena_app::TuiLaunchArgs {
    ...
    agena_app::init_tui_tracing(&launch_args.config_override_expressions, &launch_args)?;
    agena_app::run_embedded(launch_args).await
}
...
async fn run_app_server(request: AppServerRequest) -> Result<(), AgenaAppError> {
    ...
    let result = match request.args.transport {
        AppServerTransport::Stdio => jsonrpc::serve_stdio(backend)
            .await
            .map_err(|err| AgenaAppError::Configuration(err.to_string())),
    };
    runtime.shutdown();
    result
}
...
fn resolve_run_options(
    ...
    let model = if let Some(model) = model {
        providers
            ...
            .map_err(|error| AgenaAppError::Configuration(error.to_string()))?
    } else {
        default_model(providers)?
    };
...
fn default_model(providers: &dyn ProviderCatalog) -> Result<agena_domain::ModelRef, AgenaAppError> {
    providers
        .default_model()
        .map_err(|error| AgenaAppError::Configuration(error.to_string()))?
        .ok_or_else(|| AgenaAppError::Configuration("no providers configured".to_owned()))
}
