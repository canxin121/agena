...
pub(crate) const SETTINGS_FILE: &str = "server-settings.json";
...
// Agena state is stored in a single SQLite database.
...
pub(crate) fn server_settings_path_candidates() -> Vec<PathBuf> {
    let mut candidates = Vec::<PathBuf>::new();
...
pub(crate) fn server_settings_path() -> PathBuf {
    select_existing_path(server_settings_path_candidates())
}
