...
mod tests {
    ...
    fn tmp(label: &str) -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("agena-lsp-{label}-{suffix}"));
        std::fs::create_dir_all(&path).unwrap();
        path
    }

    #[test]
    fn handles_extension_default_matches_everything() {
    ...
    #[test]
    fn handles_extension_is_case_insensitive_and_dot_tolerant() {
        let spec = LspServerSpec {
            name: "x".into(),
            command: "x".into(),
            args: vec![],
        ...
        assert!(spec.handles_extension("RS"));
        assert!(spec.handles_extension("ts"));
        assert!(!spec.handles_extension("py"));
    }

    #[test]
    fn resolve_root_walks_up_for_marker() {
        let workspace = tmp("walk-up");
        let inner = workspace.join("a/b/c");
        std::fs::create_dir_all(&inner).unwrap();
        std::fs::write(workspace.join("a/Cargo.toml"), "").unwrap();
        let spec = LspServerSpec {
            name: "x".into(),
            command: "x".into(),
            args: vec![],
        ...
        let root = spec.resolve_root(&inner, &workspace);
        assert_eq!(root, workspace.join("a"));
    }

    #[test]
    fn resolve_root_falls_back_to_workspace_when_no_marker_found() {
        let workspace = tmp("fallback");
        let inner = workspace.join("a");
        std::fs::create_dir_all(&inner).unwrap();
        let spec = LspServerSpec {
            name: "x".into(),
            command: "x".into(),
            ...
            env: Default::default(),
            file_extensions: vec![],
            root_markers: vec!["never-exists".into()],
            initialization_options: None,
        };
        assert_eq!(spec.resolve_root(&inner, &workspace), workspace);
    }
}
