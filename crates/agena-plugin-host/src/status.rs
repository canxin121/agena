...
mod tests {
    ...
    #[test]
    fn initial_status_is_running_with_kind() {
        let status = PluginStatus::initial("alpha", "inproc");
        assert_eq!(status.state, PluginRunState::Running);
        assert_eq!(status.kind, "inproc");
        assert!(status.pid.is_none());
        assert_eq!(status.restart_count, 0);
    }

    #[test]
    fn registry_records_started_and_exit_transitions() {
        let registry = StatusRegistry::new();
        registry.set(PluginStatus::initial("alpha", "stdio"));
        registry.record_started("alpha", Some(42), false);
        let snapshot = registry.get("alpha").expect("status present");
        assert_eq!(snapshot.state, PluginRunState::Running);
        assert_eq!(snapshot.pid, Some(42));
        assert_eq!(snapshot.restart_count, 0);

        registry.record_exit("alpha", true, Some(1), Some("boom".into()));
        let snapshot = registry.get("alpha").unwrap();
        assert_eq!(snapshot.state, PluginRunState::Restarting);
        assert_eq!(snapshot.last_exit_code, Some(1));
        assert_eq!(snapshot.last_error.as_deref(), Some("boom"));

        registry.record_started("alpha", Some(43), true);
        let snapshot = registry.get("alpha").unwrap();
        assert_eq!(snapshot.state, PluginRunState::Running);
        assert_eq!(snapshot.restart_count, 1);
        assert!(snapshot.last_restart_at_ms.is_some());
        assert!(snapshot.last_error.is_none());

        registry.record_stopped("alpha");
        assert_eq!(
            registry.get("alpha").unwrap().state,
