...
mod tests {
    ...
    fn parses_full_index() {
          ...
          "plugins": [
            {
              "id": "agena-echo",
              "name": "Echo",
              "description": "Echo plugin sample.",
              "versions": [
                {
                  "version": "0.1.2",
                  "kind": "cdylib",
                  "platform": "x86_64-unknown-linux-gnu",
                  "url": "https://example.com/echo.so",
                  "sha256": "abc"
                },
                {
                  "version": "0.1.2",
        ...
        "#;
        let parsed: RegistryIndex = serde_json::from_str(raw).unwrap();
        assert_eq!(parsed.plugins.len(), 1);
        assert_eq!(parsed.plugins[0].versions.len(), 2);
        assert_eq!(parsed.plugins[0].versions[1].platform, "any");
        assert_eq!(parsed.plugins[0].versions[0].kind, PluginKind::Cdylib);
    }
}
