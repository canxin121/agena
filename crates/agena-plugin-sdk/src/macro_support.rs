...
mod help_examples_tests {
    ...
    #[test]
    fn json_schema_for_preserves_property_named_title() {
        let schema = super::json_schema_for::<TitleFieldSchema>();
        let properties = schema
            .get("properties")
            .and_then(serde_json::Value::as_object)
    ...
    #[test]
    fn schema_usage_text_renders_object_arguments() {
        ...
        let text = schema_usage_text(&schema).expect("usage text");
        assert!(text.contains("Arguments:"));
        assert!(text.contains("`query` <string, required>: Search query."));
        assert!(text.contains("`limit` <integer, optional, default=5>"));
        assert!(text.contains("`options[]"));
        assert!(text.contains("`options[].label` <string, required>: Visible label."));
    }

    #[test]
    fn schema_usage_text_renders_basic_constraints() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "items": {
        ...
        let text = schema_usage_text(&schema).expect("usage text");
        assert!(text.contains("`items` <array<string>, required, min_items=1, max_items=3, item_min_length=2, item_max_length=8, item_pattern=^[a-z]+$>"));
    }

    #[test]
    fn schema_usage_text_lists_required_arguments_before_optional_arguments() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "gamma": {
        ...
        let text = schema_usage_text(&schema).expect("usage text");
        let required_index = text
            .find("`zeta` <string, required>")
            .expect("required arg");
        let gamma_index = text
            .find("`gamma` <string, optional>")
            .expect("first optional arg");
        let beta_index = text
            .find("`beta` <string, optional>")
            .expect("optional arg");
        assert!(required_index < gamma_index);
        assert!(gamma_index < beta_index);
    }

    #[test]
    fn schema_usage_text_renders_object_min_properties() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "settings": {
        ...
        let text = schema_usage_text(&schema).expect("usage text");
        assert!(text.contains("`settings` <object, optional, min_properties=1>"));
        assert!(text.contains("`settings.name` <string, optional>: Name."));
    }

    #[test]
    fn schema_usage_text_renders_relations_section() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "a": { "type": "string" },
        ...
        assert!(text.contains("exactly_one_of: `a`, `b`"));
        assert!(text.contains("requires `a` -> `b`"));
    }

    #[test]
    fn schema_usage_text_renders_discriminated_actions() {
        let schema = serde_json::json!({
            "oneOf": [
                {
                    "type": "object",
        ...
        assert!(text.contains("Arguments for `search`:"));
        assert!(text.contains("`query` <string, required>: Search text."));
        assert!(text.contains("Arguments for `help`:"));
        assert!(text.contains("`tool` <string, required>: Tool name."));
    }

    #[test]
    fn schema_usage_text_renders_top_level_union_variants() {
        let schema = serde_json::json!({
            "oneOf": [
                {
                    "type": "object",
                    "description": "Search path-like values.",
                    "properties": {
                ...
                {
                    "type": "object",
                    "description": "Inspect tool metadata.",
                    "properties": {
                        "tool": { "type": "string", "description": "Tool name." }
                    },
                    "required": ["tool"]
        ...
        assert!(text.contains("- Variant 1 <object>: Search path-like values."));
        assert!(text.contains("- Variant 2 <object>: Inspect tool metadata."));
        assert!(text.contains("Arguments for `variant 1`:"));
        assert!(text.contains("`path` <string, required>: Target path."));
        assert!(text.contains("Arguments for `variant 2`:"));
        assert!(text.contains("`tool` <string, required>: Tool name."));
    }

    #[test]
    fn schema_usage_text_renders_array_item_union_variants() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "items": {
        ...
        let text = schema_usage_text(&schema).expect("usage text");
        assert!(text.contains("Item variants for `items`:"));
        assert!(text.contains("- search: Search entry."));
        assert!(text.contains("- help: Help entry."));
        assert!(text.contains("Arguments for `search`:"));
        assert!(text.contains("`items[].query` <string, required>: Search text."));
        assert!(text.contains("Arguments for `help`:"));
        assert!(text.contains("`items[].tool` <string, required>: Tool name."));
    }

    #[test]
    fn schema_example_texts_generate_required_argument_examples() {
        ...
        let examples = schema_example_texts(&object_schema);
        let example = serde_json::from_str::<serde_json::Value>(&examples[0]).unwrap();
        assert_eq!(
            example,
            serde_json::json!({
                "limit": 5,
        ...
        let examples = schema_example_texts(&union_schema);
        assert_eq!(
            examples,
            vec![
                r#"search: {"action":"search","query":"<query>"}"#,
                r#"help: {"action":"help","tool":"<tool>"}"#
            ]
        );
    }

    #[test]
    fn schema_example_texts_include_array_item_examples_when_possible() {
        ...
        let examples = schema_example_texts(&array_schema);
        assert_eq!(examples.len(), 1);
        assert!(
            examples[0].contains(r#""items":[{"name":"<name>","enabled":false}]"#)
                || examples[0].contains(r#""items":[{"enabled":false,"name":"<name>"}]"#)
        );
    }

    #[test]
    fn schema_example_texts_respect_array_min_items() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "items": {
        ...
        assert_eq!(examples.len(), 1);
        assert!(examples[0].contains(r#""items":["<item>","<item>"]"#));
    }

    #[test]
    fn schema_example_texts_render_nested_union_examples() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "target": {
        ...
        for example in examples {
            let (label, json) = example.split_once(": ").unwrap();
            rendered.insert(
                label.to_string(),
    ...
    #[test]
    fn schema_usage_text_renders_nested_property_union_variants() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "value": {
                    "oneOf": [
                        ...
                        { "type": "integer", "description": "Integer form." }
                    ]
                }
            },
            "required": ["value"]
        ...
        let text = schema_usage_text(&schema).expect("usage text");
        assert!(text.contains("Variants for `value`:"));
        assert!(text.contains("- Variant 1 <string>: String form."));
        assert!(text.contains("- Variant 2 <integer>: Integer form."));
    }

    #[test]
    fn schema_example_texts_render_nested_property_union_examples() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "value": {
        ...
        assert!(
            examples
                .iter()
                .any(|example| example.starts_with("value.Variant 1 <string>: "))
        );
        assert!(
            examples
                .iter()
                .any(|example| example.starts_with("value.Variant 2 <integer>: "))
        );
        assert!(
            examples
                .iter()
                .any(|example| example.ends_with(r#"{"value":"<value>"}"#))
        );
        assert!(
            examples
                .iter()
                .any(|example| example.ends_with(r#"{"value":1}"#))
        );
    }

    #[test]
    fn schema_example_texts_render_multiple_nested_property_union_examples() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "target": {
        ...
        assert!(
            examples
                .iter()
                .any(|example| example.starts_with("target.search: "))
        );
        assert!(
            examples
                .iter()
                .any(|example| example.starts_with("target.help: "))
        );
        assert!(
            examples
                .iter()
                .any(|example| example.starts_with("mode.Variant 1 <string>: "))
        );
        assert!(
            examples
                .iter()
                .any(|example| example.starts_with("mode.Variant 2 <integer>: "))
        );
    }

    #[test]
    fn schema_example_texts_render_nested_array_item_union_examples() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "items": {
    ...
    #[test]
    fn schema_example_texts_prefer_non_null_union_examples() {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "value": {
        ...
        let examples = schema_example_texts(&schema);
        assert_eq!(examples.len(), 1);
        assert!(examples[0].starts_with("value.Variant 2 <string>: "));
        assert!(examples[0].ends_with(r#"{"value":"<value>"}"#));
    }

    #[test]
    fn schema_example_texts_render_top_level_union_examples() {
        let schema = serde_json::json!({
            "oneOf": [
                { "type": "string" },
                { "type": "integer" }
        ...
        assert!(
            examples
                .iter()
                .any(|example| example.ends_with(": \"<value>\""))
        );
        assert!(examples.iter().any(|example| example.ends_with(": 1")));
    }

    #[test]
    fn schema_example_texts_prefer_non_null_top_level_union_examples() {
        let schema = serde_json::json!({
            "oneOf": [
                { "type": "null" },
                { "type": "string" }
        ...
        let examples = schema_example_texts(&schema);
        assert!(examples.iter().all(|example| !example.ends_with(": null")));
        assert!(
            examples
                .iter()
                .any(|example| example.ends_with(": \"<value>\""))
