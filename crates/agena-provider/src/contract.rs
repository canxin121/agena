...
pub struct OAuthCallback {
    pub code: String,
    pub state: String,
    /// RFC 9207 authorization-response issuer, when returned by the server.
...
///
/// Unknown or account-specific prices remain `None` and mark the enclosing
/// estimate incomplete instead of being silently treated as free.
#[derive(Debug, Clone, Serialize, Deserialize, Default, PartialEq)]
#[serde(default)]
pub struct BillableUsageItem {
    pub kind: String,
    pub quantity: f64,
    pub unit: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Usage generated by an ordinary Agena tool that internally called an AI
/// service. It is stored on the outer assistant message while retaining its
/// actual provider/model identity for reporting and budget enforcement.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct AttributedCompletionUsage {
    pub provider_id: String,
    pub model_id: String,
    pub operation: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub request_id: Option<String>,
    pub usage: Box<CompletionUsage>,
}
...
///
/// Token categories used by `own_total_tokens` are mutually exclusive:
/// `input_tokens` excludes cache reads/writes and `output_tokens` excludes
/// reasoning. TTL-specific writes and tool-use tokens are informational
...
pub struct CompletionUsage {
    ...
    pub recorded_cost_available: bool,
    /// Source-backed local estimate when the provider did not return cost.
    pub estimated_cost: f64,
    /// At least one token/model/tool price dimension could not be resolved.
    pub cost_estimate_incomplete: bool,

    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub billable_items: Vec<BillableUsageItem>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attributed_usage: Vec<AttributedCompletionUsage>,
}
...
impl CompletionUsage {
    ...
    pub fn total_tokens(&self) -> u64 {
        self.attributed_usage
            .iter()
            .fold(self.own_total_tokens(), |total, item| {
                total.saturating_add(item.usage.total_tokens())
            })
    }

    pub fn own_cache_input_tokens(&self) -> u64 {
        self.input_tokens
            .saturating_add(self.cache_write_tokens)
            .saturating_add(self.cache_read_tokens)
    }
    ...
    pub fn cache_input_tokens(&self) -> u64 {
        self.attributed_usage
            .iter()
            .fold(self.own_cache_input_tokens(), |total, item| {
                total.saturating_add(item.usage.cache_input_tokens())
            })
    }

    pub fn has_own_usage(&self) -> bool {
        self.requests > 0
    ...
    /// Best available cost across the outer completion and every nested
    /// provider-backed ordinary-tool request.
    pub fn effective_cost_usd(&self) -> f64 {
        self.attributed_usage
            .iter()
            .fold(self.own_effective_cost_usd(), |total, item| {
                total + item.usage.effective_cost_usd()
            })
    }

    fn add_own_assign(&mut self, additional: &Self) {
        self.requests = self.requests.saturating_add(additional.requests);
        self.input_tokens = self.input_tokens.saturating_add(additional.input_tokens);
        self.output_tokens = self.output_tokens.saturating_add(additional.output_tokens);
        ...
        self.cache_write_5m_tokens = self
            ...
            .saturating_add(additional.cache_write_5m_tokens);
        self.cache_write_1h_tokens = self
            .cache_write_1h_tokens
            .saturating_add(additional.cache_write_1h_tokens);
        self.cache_read_tokens = self
            .cache_read_tokens
            .saturating_add(additional.cache_read_tokens);
        self.tool_use_tokens = self
            .tool_use_tokens
        ...
        self.cost_estimate_incomplete |= additional.cost_estimate_incomplete;
        self.billable_items
            .extend(additional.billable_items.iter().cloned());
    }

    pub fn add_assign(&mut self, additional: &Self) {
        self.add_own_assign(additional);
        self.attributed_usage
            .extend(additional.attributed_usage.iter().cloned());
    }

    /// Collapse nested requests into one aggregate without double-counting
    /// informational subset fields.
    pub fn flattened(&self) -> Self {
        let mut total = self.clone();
        total.attributed_usage.clear();
        for attributed in &self.attributed_usage {
            total.add_own_assign(&attributed.usage.flattened());
        }
        total
    }

    pub fn saturating_sub(&self, earlier: &Self) -> Self {
        let attributed_usage = self
            .attributed_usage
            .iter()
            .enumerate()
            .filter_map(|(index, item)| {
                let usage = earlier
                    .attributed_usage
                    .get(index)
                    .filter(|previous| attributed_identity(previous) == attributed_identity(item))
                    .map_or_else(
                        || (*item.usage).clone(),
                        |previous| item.usage.saturating_sub(previous.usage.as_ref()),
                    );
                usage.has_own_usage().then(|| AttributedCompletionUsage {
        ...
        Self {
            ...
            cache_write_5m_tokens: self
                .cache_write_5m_tokens
                .saturating_sub(earlier.cache_write_5m_tokens),
            cache_write_1h_tokens: self
                .cache_write_1h_tokens
                .saturating_sub(earlier.cache_write_1h_tokens),
            ...
            cost_estimate_incomplete: self.cost_estimate_incomplete,
            // Provider streaming usage does not expose a reliable incremental
            // billable-unit delta; only terminal observations carry these.
            billable_items: if earlier.billable_items.is_empty() {
                self.billable_items.clone()
            } else {
                Vec::new()
            },
            attributed_usage,
...
fn attributed_identity(item: &AttributedCompletionUsage) -> (&str, &str, &str, Option<&str>) {
    (
...
/// Exact provider-facing function name for either a gateway function or a
/// directly exposed execution tool.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash)]
#[serde(transparent)]
pub struct ModelToolFunction(String);

impl ModelToolFunction {
    ...
    pub fn gateway_function(&self) -> Option<agena_domain::ToolApiFunction> {
        agena_domain::ToolApiFunction::from_function_name(self.function_name())
    }
}

impl From<agena_domain::ToolApiFunction> for ModelToolFunction {
    fn from(value: agena_domain::ToolApiFunction) -> Self {
