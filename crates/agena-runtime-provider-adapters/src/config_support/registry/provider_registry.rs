...
use super::{
    ...
    Path, ProviderAdapterDefinition, ProviderAuthConfig, ProviderModelRoute, ProviderRegistry,
...
fn config_adapter_error(error: crate::ProviderError) -> ConfigError {
    ConfigError::Validation(error.to_string())
}
...
pub fn build_provider_registry_from_configs(
    ...
    for (provider_id, resolved) in providers {
        if !resolved.enabled {
            continue;
        }

        let client =
            ProviderRegistry::build_http_client(agena_provider::ProviderHttpClientConfig {
...
pub(crate) fn build_provider(
    ...
    if let Some(provider_record) = catalog.map(|snapshot| snapshot.merged_models()) {
        Ok(CatalogedModelsProvider::new(
            provider,
            provider_record.into_provider_catalog(),
        ))
    } else {
        Ok(provider)
    }
            ...
            ProviderAuthConfig::Api(_) => {
                ...
                let provider = OpenAiResponsesAdapter::new_managed_with_options(
                    ...
                    OpenAiResponsesAdapterOptions {
                        ...
                        .unwrap_or(agena_provider::CapabilityFamily::OpenAi),
                ...
                agena_provider::CredentialIssuer::OpenaiChatgpt => {
                    ...
                    let provider = OpenAiResponsesAdapter::new_managed_with_options(
                        ...
                        OpenAiResponsesAdapterOptions {
                            ...
                            auth_header: adapter.options.auth_header.clone(),
                            auth_scheme: adapter.options.auth_scheme.clone(),
                            capability_family: agena_provider::CapabilityFamily::OpenAi,
                            extra_headers: http_adapter_extra_headers(
                                adapter,
                                Some(http_adapter_default_user_agent(
                                    client_identity,
