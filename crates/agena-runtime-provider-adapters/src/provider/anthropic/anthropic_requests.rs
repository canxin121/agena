use super::{
    ...
    AnthropicTextBlock, AnthropicUsage, AttachmentItem, AttachmentKind, CompletionRequest,
...
impl AnthropicAdapter {
    ...
    pub(crate) fn merge_tool_provider_options(
        ...
        let extra = extra.as_object().ok_or_else(|| {
            ProviderError::Config(format!(
                "anthropic provider-native tool `{tool_label}` provider_options must be a JSON object"
            ))
        })?;
    ...
    pub(crate) fn tools(&self, request: &CompletionRequest) -> Result<Vec<Value>, ProviderError> {
        let mut tools = self.tool_api_function_tools(request);
        tools.extend(self.provider_native_tools(request)?);
        Ok(tools)
    }
    ...
    pub(crate) fn tool_api_function_tools(&self, request: &CompletionRequest) -> Vec<Value> {
        ...
        for tool in &request.tool_api_functions {
            ...
            map.insert("input_schema".to_owned(), tool.input_schema.clone());
        ...
        for binding in request.provider_native_tools.bindings() {
            if binding.route != ProviderNativeToolRoute::ProviderHosted {
                return Err(ProviderError::Config(format!(
                    "anthropic provider-native tool `{}` only supports `provider_hosted` routes in the current runtime",
            ...
            match binding.tool {
                ProviderNativeToolKind::WebSearch => {
                        ...
                        || config.search_context_size.is_some()
                    {
                        return Err(ProviderError::Config(
                            "anthropic provider-native tool `web_search` only supports domain filters, user_location, and provider_options in the current runtime".to_owned(),
                        ));
                    }
                    if !config.allowed_domains.is_empty() && !config.blocked_domains.is_empty() {
                        return Err(ProviderError::Config(
                            "anthropic provider-native tool `web_search` cannot set both `allowed_domains` and `blocked_domains` in the same request".to_owned(),
                        ));
                    }
                    let mut map = serde_json::Map::new();
                    map.insert(
                ...
                other => {
                    return Err(ProviderError::Config(format!(
                        "anthropic provider-native tool `{}` is not supported by the current runtime",
                        other.config_key()
                    )));
                }
            }
