...
pub fn default_feature_flags() -> HashMap<String, bool> {
    HashMap::from([
...
impl GitlabProvider {
    ...
    fn listed_model_id(model_id: &str) -> String {
        agena_provider::normalized_catalog_model_id(model_id.trim().trim_start_matches("gitlab/"))
    }
    ...
    async fn fetch_proxy_models(&self) -> Result<Vec<Model>, ProviderError> {
    ...
    async fn get_direct_access_token(
        &self,
        force_refresh: bool,
    ) -> Result<DirectAccessToken, ProviderError> {
        if !force_refresh {
            let cached = self
                .direct_access_cache
                .lock()
                .map_err(|_| {
                    ProviderError::Internal("gitlab direct access cache lock poisoned".to_owned())
                })?
                .clone();
    ...
    fn invalidate_direct_access_cache(&self) -> Result<(), ProviderError> {
        *self.direct_access_cache.lock().map_err(|_| {
            ProviderError::Internal("gitlab direct access cache lock poisoned".to_owned())
        })? = None;
        Ok(())
    ...
    fn prompt_cache_direct_access_shape(&self) -> Option<agena_provider::PromptCacheShape> {
        ...
        let fields = (!route_headers.is_empty()).then(|| {
            (
                ...
                agena_provider::PromptCacheShape::json_field_value(&route_headers),
            )
        });
    ...
    fn prompt_cache_shape(&self, model: &ModelId) -> Option<agena_provider::PromptCacheShape> {
        ...
        let mut shape = agena_provider::PromptCacheShape::from_fields(
            ...
            [
                ...
                (
                    "feature_flags",
                    agena_provider::PromptCacheShape::json_field_value(&feature_flags),
                ),
...
fn should_retry_backend_auth(err: &ProviderError) -> bool {
    match err {
        ProviderError::HttpStatus { status, .. } => should_retry_credential(*status),
        _ => false,
    }
}
