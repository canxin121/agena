...
fn validate_auth_data(provider_id: &str, auth: &AuthData) -> Result<(), ProviderError> {
    match auth {
        AuthData::Api { key } => {
...
fn missing_oauth_credential_error(provider_id: &str) -> ProviderError {
    ProviderError::Config(format!("{provider_id} oauth credential not found"))
}
