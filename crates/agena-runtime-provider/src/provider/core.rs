...
pub trait ModelRuntime: Send + Sync {
    ...
    fn model_capabilities(&self, model: &ModelId) -> ModelCapabilities {
        match self.capability_family() {
            Some(family) => CapabilityResolver::capabilities_for_family(
                agena_provider::default_capability_registry(),
                family,
                model.as_ref(),
            ),
    ...
    fn model_metadata(&self, model: &ModelId) -> ModelMetadata {
        match self.capability_family() {
            Some(family) => agena_provider::default_model_metadata_registry()
                .metadata_for_family(family, model.as_ref()),
    ...
    fn model_thinking_modes(&self, model: &ModelId) -> Vec<ModelThinkingMode> {
        match self.capability_family() {
            Some(family) => agena_provider::default_model_mode_registry()
                .thinking_modes_for_family(
                    family,
                    None,
                    model.as_ref(),
                    &self.model_metadata(model),
