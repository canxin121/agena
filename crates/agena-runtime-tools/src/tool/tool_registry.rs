...
fn tool_api_description(function: ToolApiFunction) -> &'static str {
    match function {
        ToolApiFunction::List => {
            "Enumerate the current live execution-tool inventory. The plugin argument accepts one plugin id or an array of ids with OR semantics; batch multi-plugin inventory into one call. Use this whenever the pending request asks which tools or capabilities are available or broad inventory is useful; never answer inventory questions from memory. Each result contains an exact current-session identifier. Use tools_help before the first tools_call when that tool's complete live input contract is not already established. Execution-tool identifiers are not provider function names. Supports pagination and tag filters, with no configured maximum for limit. Call this function directly with well-formed JSON arguments (for example {\"plugin\":[\"agena.fs\",\"agena.monitor\"],\"limit\":200}); never put its name inside tools_call.arguments.tool."
        }
        ToolApiFunction::Search => {
            "Locate live Agena execution tools by needed capability, exact or partial name, summary, or tag. Search is precision-filtered: limit only caps relevant matches and is never filled with weak candidates, so fewer results or zero is a valid outcome. If no tool matches, revise the capability terms or narrow/broaden the plugin scope instead of choosing an unrelated tool. The query argument accepts one string or an array of strings, and plugin accepts one plugin id or an array with OR semantics; batch independent searches and multi-plugin scopes into one call. There is no configured maximum for limit. Use this before naming a tool unless an exact current-session identifier is already established. If a prior tools_call reported an unknown tool, search instead of choosing a suggestion and guessing its schema. Use exact returned names in tools_help, then tools_call. Execution-tool names never become provider function names. Pass valid JSON arguments; call this function directly, never inside tools_call.arguments.tool."
        }
        ToolApiFunction::Help => {
            "Get live input schemas, required fields, examples, and usage notes for exact Agena execution-tool identifiers returned by tools_list or tools_search. The `tool` argument accepts one execution-tool name or an array of names; batch help for several known tools into one call. Values must name execution tools (for example `fs.read`), never Tool API function names. Use this before the first tools_call unless the complete current contract is already established by reusable or embedded help. This function describes tools but does not run or authorize them. Arguments must be valid JSON with correct syntax."
        }
        ToolApiFunction::Tags => {
            "List tags used by the Agena execution tools available in this session. The plugin argument accepts one plugin id or an array with OR semantics, so one call can summarize tags across several plugin catalogs. There is no configured maximum for limit. Use returned tags to filter tools_list or tools_search. This function does not run an execution tool. Call it directly with valid JSON arguments; never route it through tools_call."
        }
        ToolApiFunction::Call => {
            "Run one known Agena execution tool. Never invent the tool name or guess its input schema. The arguments of this function must be a single JSON object with exactly two fields: `tool` (a string naming an execution tool) and `input` (an object holding that tool's arguments). One tools_call keeps one durable execution target; when several calls are independent, emit multiple complete tools_call function calls together in the same assistant response so the runtime can batch them. Never send a JSON-encoded string or any other non-object as the arguments value - the arguments value itself must be the `{ \"tool\": ..., \"input\": { ... } }` object, never a string that merely contains that JSON, and never an extra wrapper layer. Set `tool` to an exact current-session execution-tool identifier returned by tools_list or tools_search (for example `fs.read`); `tool` must be an execution tool, never a Tool API function name such as `tools_call`, `tools_help`, or `tools_list`. Set `input` to one complete argument object that exactly matches the selected tool's live tools_help contract - correct field names, types, and values; omit nothing required. The whole arguments value must be valid JSON: correct quoting and escapes (no invalid escapes such as `\\|`), no stray control characters, no truncation. If the tool is unknown, return to tools_search; if validation embeds complete help, read it and retry tools_call directly. The provider function name is always tools_call; all ordinary tools execute through this gateway. When the transport rejects a call, read the correction and re-emit with an exact tool name and valid arguments."
        }
        ToolApiFunction::PluginsList => {
            "Enumerate the current live plugin inventory. The plugin argument accepts one exact or compact plugin id or an array of ids with OR semantics, so several known plugin records can be fetched together. Each result lists the plugin id, version, summary, metadata tags, and the number of tools it publishes. There is no configured maximum for limit. Use this whenever the request asks which plugins or extensions are available; never answer plugin inventory from memory. Call this function directly with valid JSON arguments; never route it through tools_call."
        }
        ToolApiFunction::PluginsSearch => {
            "Locate live plugins by capability, exact or partial plugin id, summary, or tag. Search is precision-filtered: limit caps relevant matches rather than filling the page, so an empty result is valid and should trigger a revised query or scope, not selection of an unrelated plugin. The query argument accepts one string or an array of strings for independent batched searches; plugin accepts one plugin id or an array to constrain the catalog with OR semantics; limit has no configured maximum. Useful before choosing a plugin or understanding which plugin owns a tool. Returns the same fields as plugins_list. Call this function directly with valid JSON arguments; never route it through tools_call."
        }
        ToolApiFunction::PluginsTags => {
            "List tags used by the plugins loaded in this session. The plugin argument accepts one plugin id or an array with OR semantics, allowing one tag summary across several selected plugins. There is no configured maximum for limit. Use returned tags to filter plugins_list or plugins_search. Call this function directly with valid JSON arguments; never route it through tools_call."
        }
    }
}
