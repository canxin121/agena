...
impl ToolOutputTruncator {
    ...
    pub(crate) fn apply(&self, mut execution: ToolPayloadExecution) -> ToolPayloadExecution {
        ...
        let output_text =
            crate::truncate_tool_output_text(&execution.view.output_text, self.policy.max_chars);
        ...
        match &mut execution.output {
            ToolPayloadOutput::Shell { output, .. } => {
                if let Some(text) = output.as_mut() {
                    *text = crate::truncate_tool_output_text(text, self.policy.max_chars);
                }
            }
            ...
            ToolPayloadOutput::ApplyPatch {
                ...
                *inverse_patch =
                    crate::truncate_tool_output_text(inverse_patch, self.policy.max_chars);
                *diff = crate::truncate_tool_output_text(diff, self.policy.max_chars);
            }
