    ...
    E: ConfigEnvironment,
{
    ...
    pub fn load(&self, request: &LoadConfigRequest) -> Result<ConfigResolution, ConfigError> {
        let config_path = agena_runtime::default_config_path(&self.env);
        let workspace_root = request
            .workspace_root
            .clone()
            .unwrap_or_else(agena_runtime::default_workspace_root);
        let project_config_path = agena_runtime::project_config_path(workspace_root.as_path());

        agena_runtime::reject_unsupported_mode_environment(&self.env)?;
        ...
        Ok(ConfigResolution {
            config,
            meta: agena_runtime::ConfigResolutionMeta::from_layer_presence(
                config_path,
                file_state.found,
                project_config_path,
