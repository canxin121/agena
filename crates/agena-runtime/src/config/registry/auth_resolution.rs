...
pub(crate) fn http_adapter_default_user_agent(
    ...
    credential_user_agent(auth, default_model).unwrap_or_else(|| match adapter {
        HttpAdapterKind::OpenAi => agena_runtime::codex_user_agent(),
        HttpAdapterKind::Anthropic => agena_runtime::claude_code_api_user_agent(),
        HttpAdapterKind::Gemini => agena_runtime::gemini_cli_user_agent(default_model),
    })
}
...
pub(crate) fn credential_user_agent(
    ...
    match config.issuer() {
        agena_provider::CredentialIssuer::OpenaiChatgpt => Some(agena_runtime::codex_user_agent()),
        agena_provider::CredentialIssuer::GoogleAdc => {
            Some(agena_runtime::gemini_cli_user_agent(default_model))
        }
        _ => None,
    }
}
