...
pub(super) fn router_thinking_modes(
    ...
    for level in &thinking.levels {
        if let Some(effort) = effort_for_mode_name(level) {
            agena_provider::insert_catalog_thinking_effort(&mut modes, effort);
        }
    }
...
pub(super) fn codex_thinking_modes(
    ...
    for level in supported_reasoning_levels.unwrap_or_default() {
        ...
        if !modes.iter().any(|existing| {
            agena_provider::configured_thinking_payload_selector(existing)
                == agena_provider::configured_thinking_payload_selector(&mode)
        }) {
            modes.push(mode);
        }
    }
    ...
    if let Some(default_selector) = default_selector
        && !modes.iter().any(|mode| {
            agena_provider::configured_thinking_payload_selector(mode).as_deref()
...
fn codex_configured_thinking_mode(selector: &str) -> Option<ConfiguredModelThinkingMode> {
    ...
    effort_for_mode_name(selector).map(agena_provider::catalog_thinking_mode_for_effort)
}
...
mod tests {
    ...
    fn codex_catalog_default_is_stored_on_the_matching_mode() {
        ...
        assert_eq!(
            modes
                ...
                .filter_map(agena_provider::configured_thinking_payload_selector)
                .collect::<Vec<_>>(),
            vec!["high"]
        );
    }
    ...
    fn codex_catalog_materializes_an_unlisted_off_default() {
        ...
        assert_eq!(
            agena_provider::configured_thinking_payload_selector(&modes[0]).as_deref(),
            Some("off")
        );
    }
}
