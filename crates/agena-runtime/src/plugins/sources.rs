...
pub(crate) fn bundled_plugin_entries() -> BTreeMap<String, ConfiguredPlugin> {
    let mut entries = BTreeMap::from([
        ...
        (
            agena_runtime::web_plugin_id().to_string(),
            static_entry(serde_json::Value::Null),
        ),
        (
            agena_runtime::memory_plugin_id().to_string(),
            static_entry(serde_json::Value::Null),
        ),
        (
...
pub(crate) fn static_plugin_registrations(
    ...
    let mut registrations = vec![
        ...
        StaticPluginRegistration::new(
            plugin_key(agena_runtime::web_plugin_id()),
            agena_runtime::new_web_plugin(),
        ),
        StaticPluginRegistration::new(
            plugin_key(agena_runtime::memory_plugin_id()),
            agena_runtime::new_memory_plugin(),
        ),
    ];
    if crate::tool::schema_lab_builtin_enabled() {
