...
impl GeminiAdapter {
    pub fn new_managed_with_options(
        ...
        let user_agent = agena_runtime::gemini_cli_user_agent(default_model.as_ref());
    ...
    pub(super) fn completion_request_headers(
        ...
        let default_user_agent = agena_runtime::gemini_cli_user_agent(self.default_model.as_ref());
        ...
        if let Some(key) = generated_user_agent {
            headers.insert(
                key,
                agena_runtime::gemini_cli_user_agent(request.model.as_ref()),
            );
        }
        headers
    }
