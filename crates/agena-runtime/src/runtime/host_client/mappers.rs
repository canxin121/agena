...
pub(super) fn agent_scope_from_str(scope: &str) -> agena_domain::AgentScope {
    match scope {
        "project" => agena_domain::AgentScope::Project,
        "user" => agena_domain::AgentScope::User,
        _ => agena_domain::AgentScope::Default,
    }
}
...
pub(super) fn agent_to_descriptor(profile: crate::agents::AgentProfile) -> HostAgentDescriptor {
    HostAgentDescriptor {
        ...
        scope: match profile.scope {
            agena_domain::AgentScope::Project => "project",
            agena_domain::AgentScope::User => "user",
            agena_domain::AgentScope::Default => "default",
        }
        .to_string(),
    }
}
...
pub(super) fn core_path_access_rule_from_sdk(
    ...
    match rule {
        agena_plugin_host::sdk::host_api::AgentPathAccessRule::Modes(modes) => {
            crate::agent::PathAccessRuleConfig::Modes(core_path_access_modes_from_sdk(modes))
        }
        agena_plugin_host::sdk::host_api::AgentPathAccessRule::Shorthand(value) => {
...
pub(super) fn sdk_path_access_rule_from_core(
    ...
    match rule {
        crate::agent::PathAccessRuleConfig::Modes(modes) => {
            agena_plugin_host::sdk::host_api::AgentPathAccessRule::Modes(
...
pub(super) fn core_tool_permission_rules_from_sdk(
    ...
    match rules {
        agena_plugin_host::sdk::host_api::AgentToolPermissionRules::Mode(mode) => {
            crate::agent::ToolPermissionRules::Mode(core_permission_mode_from_sdk(mode))
        }
        agena_plugin_host::sdk::host_api::AgentToolPermissionRules::Ordered(entries) => {
...
pub(super) fn sdk_tool_permission_rules_from_core(
    ...
    match rules {
        crate::agent::ToolPermissionRules::Mode(mode) => {
            agena_plugin_host::sdk::host_api::AgentToolPermissionRules::Mode(
                sdk_permission_mode_from_core(mode),
            )
        }
        crate::agent::ToolPermissionRules::Ordered(entries) => {
            agena_plugin_host::sdk::host_api::AgentToolPermissionRules::Ordered(
                entries
                    .into_iter()
                    .map(|(pattern, mode)| (pattern, sdk_permission_mode_from_core(mode)))
                    .collect(),
...
pub(super) fn core_permission_mode_from_sdk(
    ...
    match mode {
        agena_plugin_host::sdk::host_api::AgentPermissionMode::Allow => {
            agena_domain::PermissionMode::Allow
        }
        agena_plugin_host::sdk::host_api::AgentPermissionMode::Ask => {
            agena_domain::PermissionMode::Ask
        }
        agena_plugin_host::sdk::host_api::AgentPermissionMode::Deny => {
