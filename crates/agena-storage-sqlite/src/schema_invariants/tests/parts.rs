...
async fn non_trigger_schema(db: &DatabaseConnection) -> Vec<(String, String, String)> {
    db.query_all(Statement::from_string(db.get_database_backend(), "SELECT type, name, sql FROM sqlite_schema WHERE type IN ('table','index') AND sql IS NOT NULL ORDER BY type, name")).await.unwrap().into_iter().map(|row| (row.try_get("", "type").unwrap(), row.try_get("", "name").unwrap(), row.try_get("", "sql").unwrap())).collect()
}

async fn schema_cookie(db: &DatabaseConnection) -> i64 {
...
#[tokio::test]
async fn reopening_v13_installs_part_checks_without_rewriting_rows_tables_or_indexes() {
    ...
    .await
    .unwrap();
    insert(&db, 2, "text", "pending", r#"{"text":"preserved"}"#)
        .await
        .unwrap();
    let before = [snapshot(&db, 1).await, snapshot(&db, 2).await];
    let layout = non_trigger_schema(&db).await;
    let triggers = db
        .query_all(Statement::from_string(
            db.get_database_backend(),
            "SELECT name FROM sqlite_schema WHERE type = 'trigger' AND name LIKE 'agena_parts_%'",
        ))
        .await
        .unwrap();
    for row in triggers {
    ...
    let historical: Vec<_> = include_str!("../fixtures/v13_parts.sql")
        .split("\n\n")
        .map(str::trim)
        .filter(|sql| sql.starts_with("CREATE TRIGGER"))
        .collect();
    assert_eq!(historical.len(), 17);
    for sql in historical {
        execute(&db, sql, []).await.unwrap();
    }
    let bypasses = [
        "UPDATE agena_parts SET content = '{}' WHERE part_id = 1",
        "UPDATE agena_parts SET content = 'not json' WHERE part_id = 2",
        "UPDATE agena_parts SET provider_state = 'not json' WHERE part_id = 2",
        "UPDATE agena_parts SET started_at_ms = -1 WHERE part_id = 2",
    ];
    let txn = db.begin().await.unwrap();
    for sql in bypasses {
        txn.execute(Statement::from_string(db.get_database_backend(), sql))
            .await
            .expect("historical triggers permit the reproduced bypass");
    }
    txn.rollback().await.unwrap();
    crate::initialize_schema(&db).await.unwrap();
    assert_eq!(non_trigger_schema(&db).await, layout);
    assert_eq!([snapshot(&db, 1).await, snapshot(&db, 2).await], before);
    for sql in bypasses {
        execute(&db, sql, [])
            .await
            .expect_err("refreshed v13 rejects bypass");
    }
    let cookie = schema_cookie(&db).await;
    crate::initialize_schema(&db).await.unwrap();
    assert_eq!(schema_cookie(&db).await, cookie);
    let version: i64 = db
