impl App {
    ...
    pub(crate) fn render_model_catalog_studio_overlay(
        ...
        let query_label = if dialog.presentation.query().trim().is_empty() {
            ui_text::t(&self.i18n, "overlay-model-catalog-query-all")
        } else {
            dialog.presentation.query().to_owned()
        };
        let summary = self.i18n.text_args(
            "overlay-model-catalog-summary",
            &agena_tui::fl_args!(
                "query" => query_label,
            "start" => dialog.presentation.offset().saturating_add(1) as i64,
            "end" => dialog.presentation.offset().saturating_add(dialog.presentation.list().items.len()) as i64,
            "total" => dialog.presentation.total() as i64,
        ...
        let list_items = dialog
            ...
            .map(|entry| {
                build_detail_two_line_list_item(
                    sanitize_display_text(entry.label.as_str()).into(),
                    Some(sanitize_display_text(entry.subtitle.as_str()).into()),
                    Style::default().fg(agena_tui_components::theme::muted_color()),
                )
            })
            .collect::<Vec<_>>();
        ...
        let spec = ListWorkbenchDialogSpec::new(
            ...
            if dialog.presentation.loading() {
            ...
            } else if dialog.presentation.list().items.is_empty() {
                ...
                ListWorkbenchPanelState::items(
                    ...
                    Some(ui_text::t(&self.i18n, "overlay-model-catalog-entries").into()),
                    list_items.as_slice(),
                    Some(dialog.presentation.list().selected),
                    selection_highlight_style(),
                    ">> ".into(),
                )
            },
