...
mod tests {
    ...
    fn focus_cycles_in_declared_order() {
        let mut state = DashboardSelectionState::new(
            [Focus::Top, Focus::Left, Focus::Right],
            Focus::Top,
            0,
            0,
            0,
    ...
    #[test]
    fn per_pane_selection_helpers_update_independent_cursors() {
        let mut state = DashboardSelectionState::new(
            [Focus::Top, Focus::Left, Focus::Right],
            Focus::Top,
            0,
        ...
        state.move_top(3, 2);
        state.move_left(5, 4);
        state.move_right(2, 10);

        assert_eq!(state.top_selected(), 2);
        assert_eq!(state.left_selected(), 4);
        assert_eq!(state.right_selected(), 1);
    }
}
