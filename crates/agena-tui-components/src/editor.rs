...
mod tests {
    ...
    fn from_text_places_cursor_at_end() {
        let editor = Editor::from_text("hello".to_string());
    ...
    #[test]
    fn remove_range_expands_to_element_boundaries() {
        let mut editor = Editor::default();
        editor.insert_str("hello ");
        editor.insert_element("@file");
        editor.insert_str(" world");

        let element_start = editor.text().find("@file").expect("element present");
        let removed = editor.remove_range(element_start + 1, element_start + 2);

        assert_eq!(removed, "@file");
        assert_eq!(editor.text(), "hello  world");
        assert!(editor.draft_elements().is_empty());
    }
}
