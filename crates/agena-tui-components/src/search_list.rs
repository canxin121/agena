...
mod tests {
    ...
    impl SearchListInput for TestInput {
        fn text(&self) -> &str {
            self.0.as_str()
        }
    ...
    fn test_config() -> SearchListOverlayConfig {
        SearchListOverlayConfig {
            ...
            max_list_body_height: 8,
        }
    }

    #[test]
    fn search_list_overlay_navigation_keys_follow_standard_bindings() {
    ...
    #[test]
    fn search_list_overlay_filter_key_driver_reports_close_navigation_and_edits() {
        let mut overlay = SearchListOverlay::<PathBuf, SearchListNoCustom, (), TestInput>::new(
            "Title".to_string(),
            "Prompt".to_string(),
            "Footer".to_string(),
    ...
    #[test]
    fn search_list_dialog_spec_builder_preserves_title_and_highlight() {
        let spec = SearchListDialogSpec::new(
            Cow::Borrowed("Loading"),
            Style::default(),
            Cow::Borrowed(">> "),
        )
        .with_list_title(Cow::Borrowed("Matches"));

        assert_eq!(spec.loading_message.as_ref(), "Loading");
        assert_eq!(spec.list_title.as_deref(), Some("Matches"));
        assert_eq!(spec.highlight_symbol.as_ref(), ">> ");
    }

    #[test]
    fn search_list_dialog_state_reports_loading_before_stale_rows() {
        let mut overlay = SearchListOverlay::<PathBuf, SearchListNoCustom, (), TestInput>::new(
            "Title".to_string(),
            "Prompt".to_string(),
            "Footer".to_string(),
        ...
        overlay.items = vec![PathBuf::from("stale.txt")];
        overlay.loading = true;

        assert!(matches!(
            overlay.dialog_state("Loading"),
            SearchListDialogState::Loading { message: "Loading" }
        ));
    }
}
