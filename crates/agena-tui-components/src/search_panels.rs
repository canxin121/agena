...
use crate::{
    Editor, EditorPanelSpec, FramedSurfaceSpec, SearchInputKeyResult, SearchListInput,
    WorkbenchTextSection, bordered_text_height,
    layout::{
...
mod tests {
    ...
    use crate::{Editor, SearchInputKeyResult};
    ...
    #[test]
    fn refresh_search_panels_overlay_filters_items_and_clamps_selection() {
        let mut overlay = SearchPanelsOverlay::new(
            "Title".to_string(),
            "Prompt".to_string(),
            "Empty".to_string(),
            "Footer".to_string(),
            Editor::from_text("beta".to_string()),
            false,
            (),
        );
        overlay.all_items = vec![
            TestItem {
                id: "alpha",
                search_text: "alpha",
            },
        ...
        refresh_search_panels_overlay(&mut overlay, |item, query| item.search_text.contains(query));

        assert_eq!(overlay.items.len(), 1);
        assert_eq!(overlay.selected, 0);
        assert_eq!(overlay.selected_item().map(|item| item.id), Some("beta"));
    }

    #[test]
    fn search_panels_overlay_selection_helpers_follow_item_bounds() {
        let mut overlay = SearchPanelsOverlay::new(
            "Title".to_string(),
            "Prompt".to_string(),
            "Empty".to_string(),
        ...
        overlay.move_selection_page(1, 10);
        assert_eq!(overlay.selected, 2);

        overlay.move_selection_home();
        assert_eq!(overlay.selected, 0);

        overlay.move_selection_end();
        assert_eq!(overlay.selected, 2);
    }

    #[test]
    fn search_panels_dialog_state_reports_loading_empty_and_selected() {
        let mut overlay = SearchPanelsOverlay::new(
            "Title".to_string(),
            "Prompt".to_string(),
            "Empty".to_string(),
        ...
        assert!(matches!(
            overlay.dialog_state("Loading"),
            SearchPanelsDialogState::Loading { message: "Loading" }
        ));

        overlay.loading = false;
        assert!(matches!(
            overlay.dialog_state("Loading"),
            SearchPanelsDialogState::Empty { message: "Empty" }
        ));
        ...
        match overlay.dialog_state("Loading") {
            SearchPanelsDialogState::Selected(item) => assert_eq!(item.id, "one"),
            _ => panic!("expected selected item state"),
        }
    }

    #[test]
    fn search_panels_overlay_navigation_keys_follow_standard_bindings() {
        let mut overlay = SearchPanelsOverlay::new(
            "Title".to_string(),
            "Prompt".to_string(),
            "Empty".to_string(),
        ...
        assert!(
            overlay.handle_navigation_key(KeyEvent::new(KeyCode::Down, KeyModifiers::NONE), 10,)
        );
        assert_eq!(overlay.selected, 1);

        assert!(
            overlay
                .handle_navigation_key(KeyEvent::new(KeyCode::PageDown, KeyModifiers::NONE), 10,)
        );
        assert_eq!(overlay.selected, 2);

        assert!(
            overlay.handle_navigation_key(KeyEvent::new(KeyCode::Home, KeyModifiers::NONE), 10,)
        );
        assert_eq!(overlay.selected, 0);

        assert!(
            !overlay.handle_navigation_key(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE), 10,)
        );
    }

    #[test]
    fn search_panels_overlay_filter_key_driver_reports_close_navigation_and_edits() {
        let mut overlay = SearchPanelsOverlay::new(
            "Title".to_string(),
            "Prompt".to_string(),
            "Empty".to_string(),
        ...
        assert_eq!(
            overlay.handle_filter_input_key(KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE), 10),
            SearchInputKeyResult::Close
        );
    ...
    fn search_panels_dialog_spec_builder_preserves_bounds_and_titles() {
        ...
        assert_eq!(spec.target_width, 90);
        assert_eq!(spec.left_panel_title.as_ref(), "Events");
        assert_eq!(spec.left_panel_min_body_height, 4);
        assert_eq!(spec.left_panel_max_body_height, 8);
        assert_eq!(spec.loading_message.as_ref(), "Loading");
    }
}
