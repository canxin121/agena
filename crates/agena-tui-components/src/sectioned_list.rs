...
#[cfg(test)]
mod tests {
    use super::{SectionedListFocus, SectionedListSection, SectionedListState};
    ...
    impl SectionedListSection for TestSection {
    ...
    #[test]
    fn new_clamps_section_and_item_selection() {
        let state = SectionedListState::new(
            vec![
                TestSection {
                    items: vec!["a", "b"],
                },
                TestSection { items: vec!["c"] },
            ],
            9,
            9,
            SectionedListFocus::Items,
        ...
        assert_eq!(state.selected_section_index(), 1);
        assert_eq!(state.selected_item_index(), 0);
        assert_eq!(state.selected_item(), Some(&"c"));
    }

    #[test]
    fn moving_navigation_reclamps_item_selection_for_new_section() {
        let mut state = SectionedListState::new(
            vec![
                TestSection {
                    items: vec!["a", "b"],
                },
                TestSection { items: Vec::new() },
            ],
            0,
            1,
            SectionedListFocus::Navigation,
