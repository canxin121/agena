    ...
    fn stacked_dialog_section_height_uses_declared_sizes() {
        ...
        let sections = vec![
            ...
            StackedDialogSection::TextPanel(TextPanelSection {
                height: StackedDialogSectionHeight::Fixed(4),
                spec: TextPanelSpec {
                    title: None,
                    body: &text,
                    wrap: true,
            ...
            StackedDialogSection::ListPanel(ListPanelSection {
                height: StackedDialogSectionHeight::Fixed(5),
                spec: ListPanelSpec {
                    title: None,
                    items: &items,
                    selected: Some(0),
        ...
        let total = sections
            .iter()
            .map(|section| section.height(32))
            .fold(0_u16, u16::saturating_add);
        assert_eq!(total, 11);
    }

    #[test]
    fn stacked_dialog_auto_heights_use_content_rules() {
        let text = Text::from("line one\nline two");
        let items = [ListItem::new("item-a"), ListItem::new("item-b")];
        let editor = Editor::from_text("hello\nworld".to_string());
        let sections = vec![
            StackedDialogSection::Paragraph(ParagraphSection {
                height: StackedDialogSectionHeight::AutoText { min: 1, max: 4 },
                title: None,
                borders: Borders::NONE,
                body: text.clone(),
                wrap: true,
            ...
            StackedDialogSection::TextPanel(TextPanelSection {
                height: StackedDialogSectionHeight::AutoText { min: 2, max: 6 },
                spec: TextPanelSpec {
            ...
            StackedDialogSection::EditorPanel(EditorSection {
