...
mod tests {
    ...
    #[test]
    fn suggestion_popup_lines_scroll_to_selected_item() {
        let items = [
            SuggestionPopupItem {
                prefix: None,
        ...
        let lines = build_suggestion_popup_lines(20, 2, &spec);
        assert_eq!(lines.len(), 2);
        assert!(line_text(&lines[0]).contains("second"));
        assert!(line_text(&lines[1]).contains("third"));
    }

    #[test]
    fn suggestion_popup_lines_truncate_detail_and_pad_selected_row() {
        let items = [SuggestionPopupItem {
            prefix: None,
            label: Cow::Borrowed("command"),
            detail: Some(Cow::Borrowed("very long detail text")),
        }];
        let spec = SuggestionPopupSpec {
            items: &items,
            selected: 0,
        ...
        let lines = build_suggestion_popup_lines(16, 4, &spec);
        assert_eq!(lines.len(), 1);
        let text = line_text(&lines[0]);
        assert_eq!(UnicodeWidthStr::width(text.as_str()), 16);
        assert!(text.contains("…"));
    }

    #[test]
    fn suggestion_popup_lines_keep_prefix_before_label() {
        let items = [SuggestionPopupItem {
            prefix: Some(Cow::Borrowed("#7  ")),
            label: Cow::Borrowed("remember this prompt"),
            detail: None,
        }];
        let spec = SuggestionPopupSpec {
            items: &items,
            ...
            selected_style: Style::default(),
            prefix_style: Style::default(),
            selected_prefix_style: Some(Style::default()),
            label_style: Style::default(),
            detail_style: Style::default(),
            pad_selected_row: false,
        };

        let lines = build_suggestion_popup_lines(20, 4, &spec);
        assert_eq!(lines.len(), 1);
        assert!(line_text(&lines[0]).starts_with("> #7  remember"));
    }
}
