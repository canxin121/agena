...
mod tests {
    ...
    #[test]
    fn header_body_footer_surface_insets_body_and_footer() {
        let area = Rect {
            x: 0,
            y: 0,
            width: 20,
        ...
        let layout = layout_header_body_footer_surface(area, pane_header_height(area.height), 2, 1);

        assert_eq!(layout.header.height, 2);
        assert_eq!(layout.body, Rect::new(1, 2, 18, 6));
        assert_eq!(layout.footer, Rect::new(1, 8, 18, 2));
    }

    #[test]
    fn composer_surface_layout_splits_status_items_popup_and_editor() {
        let area = Rect::new(0, 0, 20, 8);
        let layout = layout_composer_surface(area, 1, 1, 2);

        assert_eq!(layout.status, Some(Rect::new(0, 0, 20, 1)));
        assert_eq!(layout.outer, Rect::new(0, 1, 20, 7));
        assert_eq!(layout.inner, Rect::new(1, 2, 18, 5));
        assert_eq!(layout.items, Some(Rect::new(1, 2, 18, 1)));
        assert_eq!(layout.popup, Some(Rect::new(1, 3, 18, 2)));
        assert_eq!(layout.editor, Rect::new(1, 5, 18, 2));
    }

    #[test]
    fn composer_surface_layout_clamps_popup_to_leave_editor_space() {
        let area = Rect::new(0, 0, 20, 5);
        let layout = layout_composer_surface(area, 0, 1, 10);

        assert_eq!(layout.inner, Rect::new(1, 1, 18, 3));
        assert_eq!(layout.items, Some(Rect::new(1, 1, 18, 1)));
        assert_eq!(layout.popup, Some(Rect::new(1, 2, 18, 1)));
        assert_eq!(layout.editor, Rect::new(1, 3, 18, 1));
    }
}
