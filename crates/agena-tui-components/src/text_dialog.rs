...
mod tests {
    ...
    use super::{LineTextDialogSpec, TextDialogLine, text_dialog_body_height};

    #[test]
    fn text_dialog_body_height_wraps_each_styled_line() {
        let text = Text::from(vec![
            Line::from(vec![Span::raw("abcd"), Span::raw(" efgh")]),
            Line::from(Span::raw("ij")),
        ]);

        assert_eq!(text_dialog_body_height(&text, 4, true), 4);
    }

    #[test]
    fn text_dialog_body_height_without_wrap_counts_lines() {
        let text = Text::from(vec![
            Line::from("one"),
            Line::from("two"),
            Line::from("three"),
        ...
        assert_eq!(text_dialog_body_height(&text, 1, false), 3);
    }

    #[test]
    fn text_dialog_line_builders_preserve_text_and_style() {
        let plain = TextDialogLine::plain("plain");
        assert_eq!(plain.text.as_ref(), "plain");
        assert_eq!(plain.style, Style::default());

        let styled = TextDialogLine::styled(
            "styled",
            Style::default()
                .fg(Color::Cyan)
    ...
    #[test]
    fn line_text_dialog_spec_builder_preserves_footer_and_scroll() {
        let lines = [TextDialogLine::plain("line")];
        let spec = LineTextDialogSpec::new("Title".into(), &lines, 76)
            .with_body_height_bounds((2, 10))
            .with_body_scroll((3, 0))
            .with_footer(
                "Footer".into(),
                (1, 2),
        ...
        assert_eq!(spec.body_height_bounds, (2, 10));
        assert_eq!(spec.body_scroll, Some((3, 0)));
        assert_eq!(spec.footer.as_deref(), Some("Footer"));
        assert_eq!(spec.footer_alignment, Some(Alignment::Right));
    }
}
