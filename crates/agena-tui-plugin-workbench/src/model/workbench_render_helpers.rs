pub fn render_plugin_list_page(frame: &mut Frame, area: Rect, dialog: &PluginWorkbenchOverlay) {
    ...
    lines.push(Line::from(Span::styled(
        fixed_columns(
            &[
                ...
                (
                    dialog.i18n.text("plugin-workbench-tab-operations").as_str(),
                    10,
                ),
    ...
    if dialog.list.visible_len() == 0 {
        ...
        for visible_row in 0..dialog.list.visible_len() {
            ...
            let Some(plugin) = dialog.plugins.iter().find(|plugin| plugin.plugin_id == key) else {
                continue;
            };
            ...
            let line = format!(
                ...
                fixed_columns(
                    &[
                        ...
                        (plugin.operations.len().to_string().as_str(), 10),
...
pub fn render_plugin_detail_page(frame: &mut Frame, area: Rect, dialog: &PluginWorkbenchOverlay) {
    ...
    let body = match dialog.navigation.detail_tab {
        ...
        PluginDetailTab::Operations => plugin_operations_text(dialog, plugin),
...
use super::{
    ...
    plugin_diagnostics_text, plugin_header_text, plugin_logs_text, plugin_operations_text,
