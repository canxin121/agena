...
/// Current TUI view of a canonical `tool_call` part.
///
/// `operation` contains only durable facts (`RawOutput` included). The
/// `presentation` value is a read-time human projection supplied by the live
/// API or generated by the owning tool. Keeping the two values separate keeps
/// renderers on the canonical raw-output projection path.
#[derive(Debug, Clone)]
...
impl ToolCallView {
    ...
    pub fn details(&self) -> ToolOutput {
        let raw = self.raw_output().cloned().unwrap_or_default();
        let mut details = ToolOutput::from_json_payload(raw.payload.as_ref()).unwrap_or_default();
    ...
    pub fn model_text(&self) -> String {
        self.raw_output()
            .map(|raw| match raw.payload.as_ref() {
    ...
    pub fn title(&self) -> &str {
        if self.presentation.title.trim().is_empty() {
            self.operation.invocation.name.as_str()
        } else {
    ...
    pub fn summary(&self) -> &str {
        self.presentation.summary.as_str()
    }
    ...
    pub fn attachments(&self) -> &[agena_domain::AttachmentItem] {
        self.raw_output()
            .map(|raw| raw.attachments.as_slice())
            .unwrap_or_default()
    }
