...
impl BundledToolInput {
    ...
    /// if the invocation does not name a bundled tool. This adapter is kept
    /// in the execution layer so persisted messages remain plugin-neutral.
...
impl BundledToolOutput {
    ...
    /// payload for the named first-party tool back into the execution-layer
    /// enum. This is intentionally name-driven so the persisted message shape
    /// does not store a separate source/type discriminator for first-party
    /// tools.
