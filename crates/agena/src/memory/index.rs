...
mod tests {
    ...
    use super::{MemoryIndex, MemorySearchDocument};

    #[test]
    fn search_supports_english_and_cjk_queries() {
        let temp = tempdir().expect("tempdir");
        let index = MemoryIndex {
            dir: temp.path().join("index"),
        };
        let docs = vec![
            MemorySearchDocument::new(
                "merge_freeze".to_string(),
                "merge freeze".to_string(),
                "release calendar".to_string(),
                Some("project".to_string()),
                "Merge freeze begins on 2026-03-05 for the mobile release.".to_string(),
                "/tmp/merge_freeze.md".to_string(),
            ),
            MemorySearchDocument::new(
                "中文".to_string(),
                "中文规范".to_string(),
                ...
                "提交前检查冻结窗口和文档更新。".to_string(),
                "/tmp/cn.md".to_string(),
            ),
        ];
        index.replace_documents(&docs).expect("index rebuild");

        let english = index.search("mobile release", 5).expect("english query");
        assert_eq!(
            english.first().map(|doc| doc.id.as_str()),
            Some("merge_freeze")
        );

        let chinese = index.search("冻结窗口", 5).expect("cjk query");
        assert_eq!(chinese.first().map(|doc| doc.id.as_str()), Some("中文"));
    }
}
