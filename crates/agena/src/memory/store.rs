...
mod tests {
    ...
    fn tmp_store(label: &str) -> MemoryStore {
        let suffix = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("agena-mem-{label}-{suffix}"));
    ...
    fn list_returns_empty_when_dir_missing() {
        let store = MemoryStore::at(std::env::temp_dir().join("agena-mem-not-there-xxxxxx"));
        let entries = store.list().unwrap();
        assert!(entries.is_empty());
    }

    #[test]
    fn list_returns_parsed_entries_sorted_by_filename() {
        let store = tmp_store("list-sort");
        fs::write(
            store.dir().join("user_role.md"),
            "---\nname: user_role\ndescription: who they are\ntype: user\n---\nuser is data scientist",
        )
        .unwrap();
        fs::write(
            store.dir().join("project_freeze.md"),
            "---\nname: project_freeze\ntype: project\n---\nbody",
        )
        .unwrap();
        // Index file should be ignored.
        fs::write(store.dir().join("MEMORY.md"), "- index entry\n").unwrap();

        let entries = store.list().unwrap();
        assert_eq!(entries.len(), 2);
        assert_eq!(entries[0].frontmatter.name, "project_freeze");
        assert_eq!(entries[0].frontmatter.r#type, Some(MemoryType::Project));
        assert_eq!(entries[1].frontmatter.name, "user_role");
    }

    #[test]
    fn malformed_files_are_skipped_not_fatal() {
        let store = tmp_store("malformed");
        fs::write(store.dir().join("ok.md"), "---\nname: ok\n---\nbody").unwrap();
        fs::write(
            store.dir().join("bad.md"),
            "---\nname: bad\nno closing marker",
        )
        .unwrap();
        let entries = store.list().unwrap();
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].frontmatter.name, "ok");
    }

    #[test]
    fn missing_frontmatter_treats_whole_file_as_body() {
        let store = tmp_store("no-fm");
        fs::write(store.dir().join("plain.md"), "just body, no frontmatter\n").unwrap();
        let entry = store.get("plain").unwrap();
        assert!(entry.frontmatter.name.is_empty());
        assert!(entry.body.starts_with("just body"));
    }
    ...
    #[test]
    fn save_writes_frontmatter_then_body_and_appends_index_line() {
        ...
        let saved = store
            ...
            .unwrap();
        assert_eq!(saved.frontmatter.name, "user_role");
        assert_eq!(saved.frontmatter.r#type, Some(MemoryType::User));
        assert!(saved.body.contains("data scientist"));

        let raw = fs::read_to_string(store.dir().join("MEMORY.md")).unwrap();
        assert!(raw.contains("[User role](user_role.md)"));
    }

    #[test]
    fn forget_removes_file_and_index_line() {
        ...
        store
            .save(NewMemory {
                ...
                index_line: Some("- [tmp](tmp.md) — drop me".to_string()),
            })
            .unwrap();
        assert!(store.dir().join("tmp.md").exists());
        store.forget("tmp").unwrap();
        assert!(!store.dir().join("tmp.md").exists());
        let raw = fs::read_to_string(store.dir().join("MEMORY.md")).unwrap();
        assert!(!raw.contains("tmp.md"));
    }

    #[test]
    fn forget_missing_entry_is_an_error() {
        let store = tmp_store("forget-missing");
        let err = store.forget("nope").unwrap_err();
        assert!(matches!(err, MemoryError::NotFound(_)));
    }

    #[test]
    fn unknown_type_label_decodes_as_other() {
        let store = tmp_store("unknown-type");
        fs::write(
            store.dir().join("x.md"),
        ...
        assert_eq!(entry.frontmatter.r#type, Some(MemoryType::Other));
    }
}
