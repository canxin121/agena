...
mod tests {
    ...
    #[test]
    fn search_ast_auto_infers_json_file_language() {
        let dir = tempfile::tempdir().expect("tempdir");
        let file = dir.path().join("sample.json");
        fs::write(&file, r#"{ "count": 1, "label": "demo" }"#).expect("write");
        ...
        let output = plugin
            .invoke_search_ast(
                dir.path().to_string_lossy().as_ref(),
                CodeSearchAstInput {
                    path: "sample.json".to_string(),
                    pattern: r#"{ $$$ }"#.to_string(),
                    language: None,
                    limit: Some(5),
                },
            )
            .expect("search_ast should infer json");
        assert!(output.output_text.contains("sample.json"));
        assert!(output.output_text.contains(r#""count": 1"#));
    }

    #[test]
    fn syntax_tree_auto_infers_typescript_file_language() {
        let dir = tempfile::tempdir().expect("tempdir");
        let file = dir.path().join("sample.ts");
        fs::write(
            &file,
            "export function demo(value: number) { return value; }\n",
        )
        .expect("write");
        ...
        let output = plugin
            ...
            .expect("syntax_tree should infer typescript");
        assert!(output.output_text.contains(r#""language": "typescript""#));
        assert!(output.output_text.contains(r#""root_kind": "program""#));
    }

    #[test]
    fn syntax_tree_auto_infers_hcl_file_language() {
        let dir = tempfile::tempdir().expect("tempdir");
        let file = dir.path().join("main.tf");
        fs::write(&file, "resource \"aws_s3_bucket\" \"demo\" {}\n").expect("write");
        ...
        let output = plugin
            ...
            .expect("syntax_tree should infer hcl");
        assert!(output.output_text.contains(r#""language": "hcl""#));
    }

    #[test]
    fn search_ast_requires_language_for_directory_auto_mode() {
        let dir = tempfile::tempdir().expect("tempdir");
        fs::write(dir.path().join("sample.rs"), "fn demo() {}\n").expect("write");
        ...
        let err = plugin
            .invoke_search_ast(
                ...
                CodeSearchAstInput {
                    path: ".".to_string(),
                    pattern: "fn $NAME() { $BODY }".to_string(),
                    language: None,
                    limit: Some(5),
                },
            )
            .expect_err("directory search should require explicit language");
        assert!(
            err.to_string()
                .contains("`language` is required when `search_ast` targets a directory")
        );
    }
    ...
    fn code_tool_input_trims_search_ast_fields_at_parse_time() {
        ...
        match parsed {
            CodeToolInput::SearchAst { args } => {
                ...
                assert_eq!(args.language, Some(CodeLanguage::Javascript));
            }
