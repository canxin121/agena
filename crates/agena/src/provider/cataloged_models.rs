...
impl ModelRuntime for CatalogedModelsProvider {
    ...
    fn native_tools_config_for_adapter(
        ...
        self.target
            .native_tools_config_for_adapter(adapter_id, model)
    }
...
mod tests {
    use super::*;
    use crate::config::{ProviderNativeToolRoute, ProviderNativeToolRoutesConfig};

    struct NativeToolsRuntime {
        default_model: ModelId,
    }

    impl NativeToolsRuntime {
        fn new() -> Self {
            Self {
                default_model: ModelId::new("m"),
            }
        }
    }

    #[async_trait]
    impl ModelRuntime for NativeToolsRuntime {
        fn id(&self) -> &str {
            "test"
        }
        ...
        fn native_tools_config_for_adapter(
            ...
            assert_eq!(adapter_id.map(AdapterId::as_str), Some("openai"));
        ...
        async fn complete(
            ...
            Err(AppError::Provider("unused test runtime".to_owned()))
        }
    }

    #[test]
    fn forwards_native_tools_config_to_wrapped_provider() {
        let mut provider_record = ModelCatalogProviderRecord::default();
        provider_record.models.insert(
            "m".to_owned(),
            crate::model_catalog::CatalogModelDefinition::default(),
        );
        let provider =
            CatalogedModelsProvider::new(Arc::new(NativeToolsRuntime::new()), provider_record);

        let native_tools = provider
            .native_tools_config_for_adapter(Some(&AdapterId::new("openai")), &ModelId::new("m"));

        assert!(native_tools.enabled);
        assert_eq!(
            native_tools.routes.web_search,
