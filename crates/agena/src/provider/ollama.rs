...
mod tests {
    ...
    fn parses_ollama_tags_models() {
        ...
        let payload = OllamaTagsResponse {
            models: vec![OllamaTagModel {
                name: Some("llama3.2:latest".to_owned()),
                model: None,
        ...
        let models = provider.parse_models(payload);

        assert_eq!(models.len(), 1);
        assert_eq!(models[0].id.as_str(), "llama3.2:latest");
        assert_eq!(models[0].provider_id.as_str(), "ollama");
        assert!(models[0].capabilities.streaming.is_supported());
    }
    ...
    fn parses_chat_response_text_tool_calls_and_usage() {
        ...
        let parsed = provider
            .completion_from_response(&ModelId::new("llama3.2"), response)
            .expect("ollama response should parse");

        assert_eq!(parsed.text, "done");
        assert_eq!(parsed.tool_calls.len(), 1);
        assert_eq!(parsed.usage.expect("usage").output_tokens, 4);
    }
}
