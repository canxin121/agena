...
mod tests {
    use super::*;
    use crate::provider::protocol_ids::openai_responses_call_id;

    fn key(value: &str) -> ProviderStreamKey {
        ProviderStreamKey::new(value).expect("key")
    }

    #[test]
    fn accumulator_aliases_item_index_and_call_ids() {
        let mut acc = ToolStreamAccumulator::default();
        let updates = acc
            .ingest(
                "openai",
                ToolStreamInput {
                    kind: ToolStreamInputKind::Start,
                    stream_key_candidates: vec![
                        key("item:item_1"),
                        key("idx:0"),
                        key("call:call_1"),
                    ],
                    provider_item_id: ProviderItemId::new("item_1"),
                    model_call_id: openai_responses_call_id("call_1"),
        ...
        assert_eq!(
            updates,
            vec![ToolStreamUpdate::Registered {
                stream_key: "item:item_1".to_owned(),
        ...
        let updates = acc
            .ingest(
                "openai",
                ToolStreamInput {
                    kind: ToolStreamInputKind::Delta,
                    stream_key_candidates: vec![key("idx:0")],
    ...
    #[test]
    fn accumulator_replaces_mismatched_snapshots() {
        let mut acc = ToolStreamAccumulator::default();
        acc.ingest(
            "openai",
            ToolStreamInput {
                ...
                model_call_id: openai_responses_call_id("call_1"),
                name: Some("lookup".to_owned()),
                arguments: Some(r#"{"query":"old"}"#.to_owned()),
            },
        )
        .expect("delta");
        ...
        let updates = acc
            .ingest(
                "openai",
                ToolStreamInput {
                    kind: ToolStreamInputKind::Finish,
                    stream_key_candidates: vec![key("idx:0")],
                    provider_item_id: None,
                    model_call_id: openai_responses_call_id("call_1"),
                    name: Some("lookup".to_owned()),
                    arguments: Some(r#"{"query":"new"}"#.to_owned()),
                },
            )
            .expect("snapshot");
        ...
        assert_eq!(
            updates,
            vec![ToolStreamUpdate::ArgumentsSnapshot {
                stream_key: "idx:0".to_owned(),
                id: Some("call_1".to_owned()),
                name: Some("lookup".to_owned()),
                arguments_json: r#"{"query":"new"}"#.to_owned(),
            }]
        );
    }

    #[test]
    fn accumulator_emits_late_metadata_updates_without_argument_changes() {
        let mut acc = ToolStreamAccumulator::default();
        acc.ingest(
            "openai",
            ToolStreamInput {
                ...
                model_call_id: None,
                name: None,
                arguments: Some(r#"{"query":"web"}"#.to_owned()),
            },
        )
        .expect("delta");
