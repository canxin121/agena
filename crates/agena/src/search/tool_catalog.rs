...
mod tests {
    ...
    fn search_supports_keyword_and_cjk_queries() {
        let docs = vec![
            ...
            ToolCatalogDocument::new(
                "fs".to_string(),
                "读取本地文件、搜索目录，并编辑工作区内容。".to_string(),
                vec![
                    "filesystem_read".to_string(),
                    "filesystem_write".to_string(),
                ],
                Some("agena.fs".to_string()),
            ),
        ];
    ...
    #[test]
    fn search_prefers_name_matches_over_description_only_matches() {
        let docs = vec![
            ToolCatalogDocument::new(
                "memory".to_string(),
                "Search, inspect, and update durable memory records.".to_string(),
                vec!["memory".to_string()],
                Some("agena.memory".to_string()),
            ),
            ToolCatalogDocument::new(
                "tools".to_string(),
        ...
        let results = search_tool_catalog(&docs, "memory", 5).expect("memory query succeeds");
        assert_eq!(results.first().map(|doc| doc.name.as_str()), Some("memory"));
    }
}
