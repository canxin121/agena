...
fn truncate_tail_to_utf8_bytes(value: &str, max_bytes: usize) -> String {
    if value.len() <= max_bytes {
        return value.to_string();
    }
    if max_bytes == 0 {
...
fn compact_tool_output_payload_for_model(
...
fn compact_json_for_model(value: &serde_json::Value, depth: usize) -> serde_json::Value {
