...
use super::shell_tools::{
    inherited_environment, powershell_command_for_windows, resolve_workdir,
...
fn process_registry(
    executor: &ToolExecutor,
) -> Result<&dyn super::monitor::MonitorService, ToolError> {
    executor
        .monitor_registry()
...
fn format_process_events(
    ...
    for event in events {
        let stream = match event.stream {
            crate::message::ProcessStream::Stdout => "out",
            crate::message::ProcessStream::Stderr => "err",
        };
