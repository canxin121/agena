...
`?` 不再打开 Help。除 Transcript 浏览模式外，`Ctrl+H` 在所有 Route、Overlay、编辑器和聊天焦点中打开上下文帮助；Transcript 浏览模式将其保留给上一条消息，仍可通过 `/help` 打开当前界面的帮助。`Ctrl+F` 和 `/find` 不再提供搜索功能。
...
Transcript 的常规 `NAV` 光标现在是一个 Vim 风格的字符光标：只高亮一个完整字形，而不是整行。CJK 宽字符和 emoji 会作为一个字符整体高亮；空渲染行保留一个可见的光标单元格。`h/l` 不会跨行，上一条／下一条消息由 `Ctrl+H`／`Ctrl+L` 负责。双击仍可显式选中完整 Markdown、活动或消息块，供 `yy`／`Enter` 等块级操作使用。
...
`v`／`V`／`Ctrl+V` 使用同一可靠的文字范围投影，但它是键盘驱动的 Visual 模式：`v` 从当前字形开始，`V` 从当前完整渲染行开始，`Ctrl+V` 选择跨行的矩形显示单元格区域；其后 `h/j/k/l`、方向键、按词/查找移动、翻页和 `Ctrl+H`／`Ctrl+L` 会扩展而非取消选区。Visual 模式下按一次 `y` 就复制当前范围并退出，`o` 可从另一端继续扩选，`gv` 恢复最近一次 Visual 范围。三种 Visual 模式可以相互切换，并保留最初的选择锚点。
...
所有导航键首先移动主光标，再由视口提供呈现。`↑/↓` 与 `j/k` 按当前屏幕上的渲染行移动，和 Vim 一样会记住所需的显示列；短行上会临时停在最后一个字形，到后续较长行时会回到原列。消息级跳转不再混入 `h/l`：使用 `Ctrl+H`／`Ctrl+L` 可直接定位相邻消息的首个可选字形。
