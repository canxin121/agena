...
Transcript 采用 cursor-first 模型。非空 Transcript 始终拥有一条可见的主光标行，`NAV` 表示普通行／块导航；`SELECT` 是叠加在同一个主光标上的鼠标文字选区，而不是另一套位置状态。视口不接受独立的浏览位置，它只负责呈现主光标和当前阅读方向上的上下文。
...
所有导航键首先移动主光标，再由视口提供呈现。`↑/↓` 与 `j/k` 保留当前上下文并采用最小滚动；大跨度的 Page、`Ctrl+U/D` 使用方向性边缘布局：向下跳转后光标靠近顶部，向上跳转后光标靠近底部。只有搜索命中或消息跳转这类显式定位才会居中。行导航会收束已有块选择或文字选区，块内任意渲染行都能成为后续操作和复制目标。即使焦点暂时位于 Composer，这条主光标行也会继续显示。
...
| 拖选 Transcript 文字 | 按终端显示单元格精确选择并自动复制；活动端同步移动主光标，选区保留到下一次导航或点击 |
