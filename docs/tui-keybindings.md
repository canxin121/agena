...
Transcript 顶部模式标记会明确显示当前唯一交互状态：非空 Transcript 通常处于 `NAV`，始终拥有一条可见渲染行或一个完整语义块作为操作目标；`SELECT` 拥有鼠标文字选区；`BROWSE` 只会出现在尚无可选择内容时。三者互斥，不会同时维护彼此脱节的光标或选区。
...
`↑/↓` 与 `j/k` 属于结构导航：它们移动当前操作目标，并只做保证目标可见所需的最小滚动。`PageUp/PageDown`、`Ctrl+U/Ctrl+D`、鼠标滚轮和应用内滚动条属于视口浏览：它们共用同一个滚动位置，并在每次移动后把单行操作目标落到新视口的中线。原来的整块选择或已完成文字选区会在开始浏览时收束为该行，因此有内容的 Transcript 不会出现整屏没有视觉焦点的状态。即使焦点暂时位于 Composer，这条 Transcript 焦点行也会继续显示。

### 鼠标

| 操作 | 行为 |
|---|---|
| 滚轮 | 浏览 Transcript，并持续把新视口的中线作为可见单行焦点 |
| 拖动右侧滚动条 | 快速定位到任意已加载位置；拖动过程中焦点行随视口中线连续移动 |
| 单击 Transcript 内容 | 选择点击位置对应的单个渲染行；后续 `j/k` 从该行继续，`y` 复制该行 |
| 双击 Transcript 内容 | 选择该行所属的完整 Markdown、活动或消息语义块；`y` 复制整块，`Enter` 可操作可折叠块 |
| 拖选 Transcript 文字 | 按终端显示单元格精确选择并自动复制；选区保留到下一次导航、点击或离开视口 |

滚轮或滚动条回到底部会恢复实时尾随；向上离开底部会暂停尾随。鼠标拖选期间始终暂停尾随，防止正在选择的内容因新输出而移动。
...
存在鼠标文字选区时，`y` 优先复制该选区；当前目标是单行时复制该渲染行，当前目标是完整语义块时复制整块。只有 Transcript 没有任何可选择内容时，`y` 和 `Enter` 才会提示尚无选择目标。
