  ...
  <div class="grid two">
    <section class="card">
      <div class="page-header" style="align-items: flex-start">
        <div>
          <h3>Marketplace</h3>
          <p class="muted">Search a registry, install plugins into the active config, and trigger upgrade or uninstall flows without leaving the web UI.</p>
        </div>
      ...
      <div class="button-row" style="margin-top: 12px; flex-wrap: wrap">
        <button class="button primary" :disabled="props.marketplaceLoading || !props.marketplaceRegistryUrl.trim()" @click="props.searchMarketplaceAction()">
          Search Registry
        </button>
        <button class="button" :disabled="props.marketplaceLoading || !props.marketplaceRegistryUrl.trim()" @click="props.searchMarketplaceAction({ refresh: true })">
          Refresh Search
        </button>
        <button class="button" :disabled="props.marketplaceLoading || !props.marketplaceRegistryUrl.trim()" @click="props.syncMarketplaceRegistryAction">
          Sync Registry
        </button>
        <button class="button" :disabled="props.marketplaceLoading || !props.marketplaceInstalled.length" @click="props.upgradeMarketplacePluginAction()">
      ...
      <div class="grid two" style="margin-top: 16px">
        ...
        <div class="stack" style="justify-content: end">
          <label class="muted"><input :checked="props.marketplaceAllowUnverified" type="checkbox" @change="emit('update:marketplaceAllowUnverified', ($event.target as HTMLInputElement).checked)" /> allow unverified</label>
          <label class="muted"><input :checked="props.marketplaceRequireSignature" type="checkbox" @change="emit('update:marketplaceRequireSignature', ($event.target as HTMLInputElement).checked)" /> require signature</label>
          <label class="muted"><input :checked="props.marketplaceRefreshIndex" type="checkbox" @change="emit('update:marketplaceRefreshIndex', ($event.target as HTMLInputElement).checked)" /> refresh index on install</label>
          <label class="muted"><input :checked="props.marketplaceCascadeUninstall" type="checkbox" @change="emit('update:marketplaceCascadeUninstall', ($event.target as HTMLInputElement).checked)" /> cascade uninstall</label>
        </div>
      </div>

      <div class="button-row" style="margin-top: 12px; flex-wrap: wrap">
        <button class="button primary" :disabled="props.marketplaceLoading || !props.marketplaceRegistryUrl.trim() || !props.marketplaceInstallSpec.trim()" @click="props.installMarketplacePluginAction">
          Install Plugin
        </button>
      </div>
        ...
        <div v-for="plugin in props.filteredMarketplacePlugins" :key="plugin.plugin_id" class="list-item">
          <div class="page-header" style="align-items: flex-start">
            <div>
              ...
              <div class="muted mono">
                latest={{ plugin.latest_version || 'n/a' }} · kind={{ plugin.latest_kind || 'n/a' }} · platform={{ plugin.latest_platform || 'n/a' }} · versions={{ plugin.version_count }}
              </div>
              <div v-if="plugin.homepage" class="muted mono">{{ plugin.homepage }}</div>
            </div>
            <span class="badge">{{ props.installedMarketplacePluginIds.has(plugin.plugin_id) ? 'installed' : 'registry' }}</span>
          </div>
          <div class="button-row" style="margin-top: 10px; flex-wrap: wrap">
            <button
              ...
              @click="emit('update:marketplaceInstallSpec', plugin.plugin_id); props.installMarketplacePluginAction()"
            >
              Install
            </button>
            <button class="button" :disabled="props.marketplaceLoading || !props.installedMarketplacePluginIds.has(plugin.plugin_id)" @click="props.upgradeMarketplacePluginAction(plugin.plugin_id)">
              Update
            </button>
            <button class="button danger" :disabled="props.marketplaceLoading || !props.installedMarketplacePluginIds.has(plugin.plugin_id)" @click="props.uninstallMarketplacePluginAction(plugin.plugin_id)">
              Uninstall
            </button>
          </div>
        </div>
