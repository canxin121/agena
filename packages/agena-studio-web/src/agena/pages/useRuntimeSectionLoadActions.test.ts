...
function createState(section: RuntimeRouteSection = 'runtime') {
  ...
  const state = {
    ...
    pluginLogs: ref<PluginLogEntry[]>([{ seq: 1, plugin_id: 'demo/plugin', level: 'info', message: 'old', timestamp_ms: 1 }]),
    ...
    selectedPlugin: ref<PluginInspect | null>({ manifest: { name: 'old' }, status: { plugin_id: 'old', kind: 'wasm', state: 'ready', restart_count: 0 } }),
    ...
    sessionExecution: ref<SessionExecutionResource | null>({
      session: { id: 10, workspace_id: 1, title: 'Old', version: 1, created_at: '', updated_at: '', message_count: 0, child_session_count: 0 },
      blocked: false,
      run_state: 'idle',
...
describe('useRuntimeSectionLoadActions', () => {
  test('loadRuntimeSection hydrates runtime state and loads session execution', async () => {
    ...
    const actions = useRuntimeSectionLoadActions(state, createDeps({
      loadRuntimeSectionData: async () => ({
        runtime: { generation: 3, loaded_at: '', workspace_root: '/repo', config_path: '/repo/.agena/config.json', config_found: true, auth_store_path: '/repo/.agena/auth.json', provider_ids: ['anthropic'], plugin_count: 1, session_runtime_available: true, watch_paths: [], reload: { enabled: true, interval_secs: 1 }, janitor: { enabled: true, interval_secs: 1 }, automation: { enabled: false, job_count: 0, recent_jobs: [] }, operator: { mcp: { server_count: 0, tool_count: 0, servers: [] }, lsp: { server_count: 0, diagnostics_count: 0, files_with_diagnostics: 0, servers: [] }, skills: { skill_count: 0, command_count: 0, skills: [], commands: [] } } },
        providers: [{ provider_id: 'anthropic', default_model: 'claude-opus-4-7', default_model_ref: 'anthropic/claude-opus-4-7' }],
        providerModels: { anthropic: [] },
        workspaces: [{ id: 2, path: '/repo', created_at: '', updated_at: '' }],
        sessions: [{ id: 22, workspace_id: 2, title: 'Session 22', version: 1, created_at: '', updated_at: '', message_count: 0, child_session_count: 0 }],
    ...
    expect(calls).toEqual([
      'replaceProviderModels:anthropic',
      'loadSessionExecution:22',
    ])
    ...
    const actions = useRuntimeSectionLoadActions(state, createDeps({
      loadSettingsSectionData: async (search) => {
        ...
        return {
          ...
          permissionRules: [{ id: 1, action_key: 'Bash:ls', subject_kind: 'builtin_tool', tool_name: 'bash', qualifier: null, mode: 'ask', scope: 'workspace', source: 'api', created_at: '', updated_at: '' }],
        }
      },
    }))
