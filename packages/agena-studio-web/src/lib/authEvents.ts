...
export const UI_AUTH_REQUIRED_EVENT = 'agena-studio.auth-required'

export const UI_AUTH_REQUIRED_STORAGE_KEY = sessionStorageKeys.auth.authRequired
...
export function readAuthRequiredFromStorage(): StoredAuthRequired | null {
  ...
  try {
    const raw = sessionStorage.getItem(UI_AUTH_REQUIRED_STORAGE_KEY)
    if (!raw) return null
...
export function clearAuthRequiredFromStorage() {
  ...
  try {
    sessionStorage.removeItem(UI_AUTH_REQUIRED_STORAGE_KEY)
  } catch {
    // ignore
  }
}
...
function persistAuthRequired(detail: AuthRequiredDetail) {
  ...
  try {
    ...
    sessionStorage.setItem(UI_AUTH_REQUIRED_STORAGE_KEY, JSON.stringify(payload))
  } catch {
    // ignore
  }
}
...
function dispatchAuthRequired(target: EventTarget, detail: AuthRequiredDetail) {
  try {
    ...
    if (typeof CustomEvent === 'function') {
      target.dispatchEvent(new CustomEvent<AuthRequiredDetail>(UI_AUTH_REQUIRED_EVENT, { detail }))
      return
    }
  } catch {
    // ignore
  ...
  try {
    ...
    const evt = new Event(UI_AUTH_REQUIRED_EVENT)
