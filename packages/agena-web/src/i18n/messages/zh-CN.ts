export default {
  ...
  help: {
    dialog: {
      ...
      sections: {
        ...
        transcriptVim: '对话记录导航',
        transcriptSelection: '对话记录选择与复制',
      ...
      shortcuts: {
        ...
        abortActiveRunDouble: '清空草稿或中断当前运行',
        cycleTheme: '切换主题（浅色 → 深色 → 系统）',
        openFiles: '打开文件',
        openTerminal: '打开终端',
        openGitPanel: '打开 Git 面板',
        openSettings: '打开设置',
        vimModes: '进入 INSERT 模式 / 返回对话记录导航',
        vimMotions: '使用可选计数按字符或可视行移动',
        vimMessages: '跳转到上一条 / 下一条消息',
        vimBounds: '跳转到第一个、指定编号或最后一个对话节点',
        vimViewport: '移动到视口位置，或把当前节点放到指定位置',
        vimScroll: '按半页、整页或单行滚动',
        vimSearch: '向前 / 向后搜索并重复匹配',
        vimStructure: '展开或折叠当前对话 part',
        vimVisual: '开始字符、行或块 Visual 选择',
        vimYank: '复制选择、当前节点或计数范围',
        vimTextObjects: '选择单词、段落、Markdown part 或完整消息',
        vimWords: '按 Unicode 单词或空白分隔的 WORD 移动',
        openPlan: '打开当前会话计划',
        vimReselect: '交换 Visual 端点 / 恢复上次选择',
        vimFind: '查找字符并重复查找',
        vimJumps: '在跳转历史中后退 / 前进',
      },
    },
  },
  mcp: {
  ...
  settings: {
    ...
    tui: {
      interfaceDescription: '管理与 TUI 共用的外观、语言、对话记录默认行为以及按工具配置的活动显示方式。',
      runtimeDescription: '管理与 TUI 运行时共用的 provider 客户端版本和会话压缩设置。',
      diagnosticsDescription: '管理 TUI Settings Studio 暴露的 tracing 与运行时诊断设置。',
      clientVersionsTitle: 'Provider 客户端版本',
      clientVersionsDescription: '固定或刷新 provider 集成使用的客户端版本。',
      refreshing: '刷新中…',
      refreshFromNpm: '从 npm 刷新',
      compactionTitle: '会话压缩',
      compactionDescription: '控制自动上下文压缩，以及压缩前保留的 token 数量。',
      activityKindsTitle: '活动类型',
      activityKindsDescription: '选择对话记录中的活动分组是否默认展开。',
      toolOverridesTitle: '工具活动覆盖项',
      toolOverridesDescription: '为单个工具覆盖活动展开状态，包括带点号的工具名称。',
      noTools: '暂无可用的工具活动覆盖项。',
      effectiveReadOnly: 'Effective 设置为只读。请选择 Global、Workspace 或 Session 来编辑持久化层。',
      fields: {
        locale: '语言区域',
        localeDescription: '运行时和 TUI 使用的语言。',
        colorScheme: '配色方案',
        colorSchemeDescription: 'TUI 使用浅色、深色或跟随系统配色。',
        graphics: '图形渲染',
        graphicsDescription: '启用或禁用终端图形渲染。',
        theme: '主题',
        themeDescription: 'TUI 主题名称或主题标识符。',
        activityDefaultExpanded: '活动默认展开',
        activityDefaultExpandedDescription: '对话记录活动分组是否默认展开。',
        codexVersion: 'Codex 客户端版本',
        codexVersionDescription: 'Codex 客户端使用的版本约束或发行版本。',
        claudeVersion: 'Claude 客户端版本',
        claudeVersionDescription: 'Claude 客户端使用的版本约束或发行版本。',
        geminiVersion: 'Gemini 客户端版本',
        geminiVersionDescription: 'Gemini 客户端使用的版本约束或发行版本。',
        autoCompaction: '自动压缩',
        autoCompactionDescription: '当上下文预算需要时自动压缩会话。',
        reservedTokens: '保留 Token 数',
        reservedTokensDescription: '触发自动压缩前保留的 Token 数量。',
        tracingFilter: 'Tracing 过滤器',
        tracingFilterDescription: '运行时使用的 tracing 过滤指令。',
        tracingDatabase: 'Tracing 数据库',
        tracingDatabaseDescription: 'Tracing 数据库写入目标。',
        tracingAdapter: 'Tracing 适配器',
        tracingAdapterDescription: '用于导出运行时事件的 tracing 适配器。',
      },
    },
    debug: {
      title: '调试工具',
  ...
  chat: {
    planViewer: {
      title: '计划',
      session: '会话 {id}',
      requiresSession: '请先打开一个会话再查看计划。',
      autorun: '自动运行',
      noAutorun: '当前没有自动运行设置',
      refresh: '刷新计划',
      loading: '正在加载计划',
      empty: '此会话当前没有计划。',
    },
    ...
    commandPalette: {
      ...
      builtIns: {
        ...
        rename: '重命名当前会话',
        timeline: '打开运行活动历史',
        settings: '打开 Agena 设置',
        model: '选择当前会话模型',
        review: '让审查 skill 检查改动',
        commit: '创建 git 提交',
        pr: '创建拉取请求',
        export: '导出已加载的对话记录',
        pager: '分页查看对话记录',
        continue: '继续中断或等待中的会话',
        compact: '压缩当前会话上下文',
        'user-input': '打开待处理的用户输入请求',
        allow: '允许一次待处理的权限',
        'allow-always': '始终允许待处理的权限',
        deny: '拒绝一次待处理的权限',
        'deny-always': '始终拒绝待处理的权限',
        attach: '附加本地文件',
        skill: '打开 skill 选择器',
        'skill-manager': '打开 skill 管理',
        download: '下载工作区文件',
        editor: '切换展开编辑器',
        image: '附加图片',
        copy: '复制已加载的对话记录',
        'copy-message': '复制最后一条 AI 消息',
        'copy-visible': '复制当前可见的对话记录',
        fork: '派生当前会话',
        children: '打开子会话',
        parent: '打开父会话',
        diagnostics: '显示会话诊断信息',
        status: '显示当前会话状态',
        usage: '打开用量分析',
        activities: '打开后台活动',
        background: '打开后台活动',
    ...
    sidebar: {
      ...
      directoriesList: {
        ...
        activity: {
          ...
          attentionTooltip: '该目录存在需要处理的会话',
          mixedTooltip: '该目录同时存在运行中和需要处理的会话',
        },
