...
async function addDirectoryEntry() {
  ...
  if (!p) return
  // Agena workspaces are created server-side by path (no settings project store).
...
function directoryForWorkspaceId(workspaceId?: number): DirectoryEntry | null {
  const wid = typeof workspaceId === 'number' ? String(workspaceId) : ''
  if (!wid) return null
  const dirs = Array.isArray(directorySessions.visibleDirectories) ? directorySessions.visibleDirectories : []
  return dirs.find((d) => String(d.id) === wid) || null
}
...
async function searchSessionHits(query: string, limit: number, signal?: AbortSignal): Promise<SidebarSearchHit[]> {
  ...
  if (!q || limit <= 0) return []
  try {
    // Agena has no sidebar-scoped search endpoint; the canonical session list
    // supports a `search` query. Directory hits are matched by workspace_id.
    const page = await chatApi.listSessions({ search: q, limit, signal })
    const out: SidebarSearchHit[] = []
    for (const session of page.sessions) {
      const sessionId = String(session?.id || '').trim()
      if (!sessionId) continue
      const directory = directoryForWorkspaceId(session.workspace_id)
