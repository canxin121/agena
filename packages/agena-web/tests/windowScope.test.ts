...
test('hasEmbeddedWorkspacePaneSearch reads canonical embed marker', () => {
  assert.equal(hasEmbeddedWorkspacePaneSearch('?agenaEmbed=1'), true)
  assert.equal(hasEmbeddedWorkspacePaneSearch('?agenaEmbed=0'), false)
...
test('isEmbeddedWorkspacePaneContext accepts embed query directly', () => {
  assert.equal(isEmbeddedWorkspacePaneContext({ agenaEmbed: '1' }), true)
  assert.equal(isEmbeddedWorkspacePaneContext({ agenaEmbed: '0' }), false)
})

test('isEmbeddedWorkspacePaneContext falls back to window search', () => {
  withWindowSearch('?windowId=win-1&agenaEmbed=1', () => {
...
test('withEmbeddedWorkspaceScopeQuery preserves embed + window scope during transient empty route query', () => {
  withWindowSearch('?windowId=win-42&agenaEmbed=1', () => {
    const next = withEmbeddedWorkspaceScopeQuery({ sessionId: 'sess-1' }, {})
    assert.equal(next.agenaEmbed, '1')
