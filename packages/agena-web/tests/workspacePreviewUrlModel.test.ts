...
test('normalizePreviewProxyBasePath normalizes preview session paths', () => {
  assert.equal(
    normalizePreviewProxyBasePath('/api/v1/workbench/preview/s/session-1'),
    '/api/v1/workbench/preview/s/session-1/',
  )
})
...
test('normalizePreviewProxyBasePath rejects non-session or direct targets', () => {
  ...
  assert.equal(normalizePreviewProxyBasePath('/api/v1/workbench/preview/proxy?target=http://localhost:5173'), '')
})

test('buildPreviewFrameSrc appends refresh token to proxy base path', () => {
  const src = buildPreviewFrameSrc('/api/v1/workbench/preview/s/session-1/', 12)
  assert.equal(src, '/api/v1/workbench/preview/s/session-1/?__oc_preview_refresh=12')
})
...
test('selected created session still builds iframe src from proxyBasePath', () => {
  const createdSession = {
    id: 'pv_demo-app',
    proxyBasePath: '/api/v1/workbench/preview/s/pv_demo-app/',
  ...
  assert.equal(src, '/api/v1/workbench/preview/s/pv_demo-app/?__oc_preview_refresh=3')
