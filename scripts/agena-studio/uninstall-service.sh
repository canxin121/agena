...
OS="$(uname -s)"
if [[ "$OS" == "Linux" ]]; then
  systemctl --user disable --now agena-studio.service >/dev/null 2>&1 || true
  rm -f "$HOME/.config/systemd/user/agena-studio.service"
  systemctl --user daemon-reload >/dev/null 2>&1 || true
elif [[ "$OS" == "Darwin" ]]; then
  PLIST_FILE="$HOME/Library/LaunchAgents/cn.cxits.agena-studio.plist"
...
echo "Agena Studio background service removed."
