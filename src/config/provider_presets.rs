...
const MODELS_DEV_PRESETS_CACHE_FILE: &str = "opencode-provider-presets.json";
const MODELS_DEV_PRESETS_PATH_ENV: &str = "AGENA_OPENCODE_PRESETS_PATH";
...
pub(super) fn apply_provider_preset(
    ...
    let preset = load_provider_presets(env)?
        ...
        .ok_or_else(|| ConfigError::InvalidProviderConfig {
            ...
            message: format!("unknown opencode preset provider `{provider_id}`"),
        })?;
...
fn read_presets_file(path: &Path) -> Result<Vec<ProviderPresetRecord>, ConfigError> {
    ...
    serde_json::from_str(text.as_str()).map_err(|err| {
        ConfigError::Validation(format!(
            "failed to parse opencode preset file {}: {err}",
            path.display()
        ))
    })
}
...
fn fetch_provider_presets() -> Result<Vec<ProviderPresetRecord>, ConfigError> {
    let runtime = tokio::runtime::Builder::new_current_thread()
        ...
        .map_err(|err| {
            ConfigError::Validation(format!(
                "failed to initialize runtime for opencode preset fetch: {err}"
            ))
        })?;
    ...
    runtime.block_on(async {
        let client = reqwest::Client::builder()
            ...
            .map_err(|err| {
                ConfigError::Validation(format!(
                    "failed to build http client for opencode preset fetch: {err}"
                ))
            })?;
        ...
        let response = client
            ...
            .map_err(|err| {
                ConfigError::Validation(format!(
                    "failed to fetch opencode preset data from models.dev: {err}"
                ))
            })?;
...
fn preset_family(preset: &ProviderPresetRecord) -> Result<PresetFamily, ConfigError> {
    ...
    match preset.npm.as_deref() {
        ...
        Some(other) => Err(ConfigError::InvalidProviderConfig {
            ...
            message: format!("unsupported opencode preset package `{other}`"),
        }),
        None => Err(ConfigError::InvalidProviderConfig {
            provider_id: preset.id.clone(),
            message: "opencode preset is missing npm metadata".to_owned(),
        }),
    }
}
...
fn apply_openai_compatible_preset(
    ...
    match provider_id {
        "openrouter" | "zenmux" | "kilo" => {
            insert_header_if_missing(raw, "HTTP-Referer", "https://opencode.ai/");
            insert_header_if_missing(raw, "X-Title", "opencode");
        }
        "vercel" => {
            insert_header_if_missing(raw, "http-referer", "https://opencode.ai/");
            insert_header_if_missing(raw, "x-title", "opencode");
        ...
        "cerebras" => {
            insert_header_if_missing(raw, "X-Cerebras-3rd-Party-Integration", "opencode");
        }
        _ => {}
    }
