...
/// Tests if a type's alignment is no stricter than its size.
//
// Most ABIs give integer types an alignment equal to their size.  The m68k
// Linux ABI, however, naturally aligns some wider integer types to fewer
// bytes.  Bitvec's pointer representation uses `align_of::<T>()` for its
// alignment invariant, so equality is unnecessarily restrictive here: the
// type's actual Rust alignment is still the only alignment required for
