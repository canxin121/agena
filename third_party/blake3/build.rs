...
fn is_x86_64() -> bool {
    // Rust's x86_64h-apple-darwin target uses the x86_64 architecture but a
    // distinct first target-triple component. Key SIMD dispatch off rustc's
    // architecture cfg instead of spelling target triples here.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if is_x86_64() || is_x86_32() {
        ...
        if is_pure() || is_x86_32() || support == NoCompiler || support == NoAVX512 {
            // AVX-512 is optional. Some cross LLVM backends accept the AVX-512
            // flags for 32-bit x86 but cannot select the generated mask
            // intrinsics. Keep the portable/SSE/AVX2 paths instead.
