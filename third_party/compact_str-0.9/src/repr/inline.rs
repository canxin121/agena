...
#[cfg(all(target_pointer_width = "32", not(target_arch = "m68k")))]
#[repr(C, align(4))]
pub(crate) struct InlineBuffer(pub(crate) [u8; MAX_SIZE]);

// m68k pointers are 2-byte aligned even though the target is 32-bit.  The
// representation must match Repr's ABI alignment for the layout assertions
// and for the inline/heap transmutations to remain sound.
#[cfg(all(target_pointer_width = "32", target_arch = "m68k"))]
#[repr(C, align(2))]
pub(crate) struct InlineBuffer(pub(crate) [u8; MAX_SIZE]);
