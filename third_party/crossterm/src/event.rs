...
static TERMINAL_RESPONSE_CAPTURE_ACTIVE: AtomicBool = AtomicBool::new(false);

/// An exclusive scope in which response-shaped control sequences are decoded
/// as [`Event::TerminalResponse`] rather than keyboard input.
///
/// A terminal query and its reply share stdin with user input, so callers must
/// begin the capture before writing the query and retain it until a protocol
/// completion marker has arrived. Only one response-bearing transaction may
/// be active at a time.
#[derive(Debug)]
pub struct TerminalResponseCapture {
    active: bool,
}

impl TerminalResponseCapture {
    /// Begin an exclusive terminal-response transaction.
    pub fn begin() -> std::io::Result<Self> {
        TERMINAL_RESPONSE_CAPTURE_ACTIVE
            .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
            .map_err(|_| {
                std::io::Error::new(
                    std::io::ErrorKind::WouldBlock,
                    "a terminal-response transaction is already active",
                )
            })?;
        Ok(Self { active: true })
    }

    /// Finish the transaction and restore ordinary keyboard decoding.
    ...
    fn release(&mut self) {
        if self.active {
            TERMINAL_RESPONSE_CAPTURE_ACTIVE.store(false, Ordering::Release);
            self.active = false;
        }
    }
}

impl Drop for TerminalResponseCapture {
    fn drop(&mut self) {
        self.release();
    }
}

pub(crate) fn terminal_response_capture_active() -> bool {
    TERMINAL_RESPONSE_CAPTURE_ACTIVE.load(Ordering::Acquire)
}
...
pub enum Event {
    ...
    Resize(u16, u16),
    /// A reply generated by a response-bearing terminal control sequence.
    ///
    /// Terminal replies share the same byte stream as keyboard input. Keeping
    /// them as first-class events prevents OSC payloads from being decoded as
    /// a sequence of Alt and character key presses.
    TerminalResponse(TerminalResponse),
}

/// A response emitted by the terminal rather than an input event generated by
/// the user.
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, PartialOrd, PartialEq, Eq, Clone, Copy, Hash)]
pub enum TerminalResponse {
    ...
    BackgroundColor {
        /// The query selector echoed by the terminal.
    ...
    /// A Device Status Report (`CSI Ps n`). Zero means "ready, no
    /// malfunction" and is used as a transaction completion marker.
    DeviceStatus(u16),
    /// A Cursor Position Report (`CSI row ; column R`). Agena uses this as a
    /// distinct startup barrier after all synchronous capability probes.
    CursorPosition {
    ...
    /// A response to a Kitty graphics protocol capability query. Agena's
    /// startup picker interprets the synchronous reply; this event guarantees
    /// that a copy delayed past its deadline is still protocol data, not text.
    KittyGraphics,
}

/// The terminal protocol used to request the default background color.
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, PartialOrd, PartialEq, Eq, Clone, Copy, Hash)]
pub enum BackgroundColorQuery {
    /// The xterm-compatible dynamic background query (`OSC 11`).
    Osc11,
    /// iTerm2's default-background extension (`OSC 4; -2`).
    Iterm2Osc4,
}
