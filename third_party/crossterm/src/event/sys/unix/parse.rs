...
use crate::event::{
    BackgroundColorQuery, Event, KeyCode, KeyEvent, KeyEventKind, KeyEventState, KeyModifiers,
    KeyboardEnhancementFlags, MediaKeyCode, ModifierKeyCode, MouseButton, MouseEvent,
    MouseEventKind, TerminalResponse,
};
        ...
        b'\x1B' => {
            if buffer.len() == 1 {
                ...
                match buffer[1] {
                    ...
                    _ => parse_alt_prefixed_event(&buffer[1..], input_available),
                }
            }
        }
...
fn parse_alt_prefixed_event(
    buffer: &[u8],
    input_available: bool,
) -> io::Result<Option<InternalEvent>> {
    parse_event(buffer, input_available).map(|event| {
            ...
            if buffer.len() == 3 {
                ...
                if !(64..=126).contains(&last_byte) {
                    ...
                    match last_byte {
                        ...
                        b'n' => return parse_csi_device_status(buffer),
...
fn parse_osc(buffer: &[u8], input_available: bool) -> io::Result<Option<InternalEvent>> {
    parse_osc_with_capture(
        buffer,
        input_available,
        crate::event::terminal_response_capture_active(),
    )
}

fn parse_osc_with_capture(
    buffer: &[u8],
    input_available: bool,
    capture_active: bool,
) -> io::Result<Option<InternalEvent>> {
    ...
    if !capture_active {
        return parse_alt_prefixed_event(&buffer[1..], input_available);
    }

    // `ESC ]` is also how an Alt+] key is encoded. Preserve Crossterm's
    // existing key behavior outside a response transaction. During a query,
    // the protocol owns this ambiguous prefix until its DSR barrier arrives,
    // even if the terminal splits the reply across OS reads.
    if buffer.len() == 2 {
        return Ok(None);
    }

    let body = &buffer[2..];
    let query = if b"11;".starts_with(body) || body.starts_with(b"11;") {
        Some((BackgroundColorQuery::Osc11, &b"11;"[..]))
    } else if b"4;-2;".starts_with(body) || body.starts_with(b"4;-2;") {
        Some((BackgroundColorQuery::Iterm2Osc4, &b"4;-2;"[..]))
    } else {
        None
    };

    let Some((query, selector)) = query else {
        // This is not a response Agena asks Crossterm to classify. Fall back
        // to the normal Alt-key path rather than swallowing arbitrary input.
        return parse_alt_prefixed_event(&buffer[1..], input_available);
    };

    let terminator_len = if buffer.ends_with(b"\x07") {
        Some(1)
    ...
    let payload_end = buffer.len() - terminator_len;
    let payload = &buffer[2 + selector.len()..payload_end];
    let Some(rgb) = payload.strip_prefix(b"rgb:") else {
    ...
    let (Some(red), Some(green), Some(blue)) = (
        parse_x11_color_component(red),
        parse_x11_color_component(green),
        parse_x11_color_component(blue),
    ) else {
        return Err(could_not_parse_event_error());
    };

    Ok(Some(InternalEvent::Event(Event::TerminalResponse(
...
fn parse_apc_with_capture(
    buffer: &[u8],
    input_available: bool,
    capture_active: bool,
) -> io::Result<Option<InternalEvent>> {
    debug_assert!(buffer.starts_with(b"\x1b_"));
    if !capture_active {
        return parse_alt_prefixed_event(&buffer[1..], input_available);
    }
    if buffer.len() == 2 {
    ...
    let body = &buffer[2..];
    if !b"G".starts_with(body) && !body.starts_with(b"G") {
        return parse_alt_prefixed_event(&buffer[1..], input_available);
    }
    ...
    Ok(Some(InternalEvent::Event(Event::TerminalResponse(
        TerminalResponse::KittyGraphics,
    ))))
}

fn parse_csi_device_status(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    debug_assert!(buffer.starts_with(b"\x1b["));
    debug_assert!(buffer.ends_with(b"n"));
    let status = std::str::from_utf8(&buffer[2..buffer.len() - 1])
        .map_err(|_| could_not_parse_event_error())?
        .parse::<u16>()
        .map_err(|_| could_not_parse_event_error())?;
    Ok(Some(InternalEvent::Event(Event::TerminalResponse(
        TerminalResponse::DeviceStatus(status),
    ))))
}
...
pub(crate) fn parse_csi_cursor_position(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    parse_csi_cursor_position_with_capture(buffer, crate::event::terminal_response_capture_active())
}

fn parse_csi_cursor_position_with_capture(
    buffer: &[u8],
    capture_active: bool,
) -> io::Result<Option<InternalEvent>> {
    // ESC [ Cy ; Cx R
    ...
    if capture_active {
        Ok(Some(InternalEvent::Event(Event::TerminalResponse(
            TerminalResponse::CursorPosition { column: x, row: y },
        ))))
    } else {
...
mod tests {
    ...
    #[test]
    fn parses_background_color_terminal_responses_before_key_decoding() {
        let response = b"\x1b]4;-2;rgb:fae0/fae0/fae0\x1b\\";
        for end in 2..response.len() {
            assert_eq!(
                parse_osc_with_capture(&response[..end], false, true).unwrap(),
                None,
                "partial OSC response of {end} bytes must remain buffered"
            );
        }
        assert_eq!(
            parse_osc_with_capture(response, false, true).unwrap(),
            Some(InternalEvent::Event(Event::TerminalResponse(
                TerminalResponse::BackgroundColor {
        ...
        assert_eq!(
            parse_osc_with_capture(b"\x1b]11;rgb:00/80/ff\x07", false, true).unwrap(),
            Some(InternalEvent::Event(Event::TerminalResponse(
                TerminalResponse::BackgroundColor {
        ...
        assert_eq!(
            parse_osc_with_capture(b"\x1b]", false, false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char(']'),
                KeyModifiers::ALT
            ))))
    ...
    #[test]
    fn parses_device_status_as_a_terminal_response() {
        assert_eq!(
            parse_event(b"\x1b[0n", false).unwrap(),
            Some(InternalEvent::Event(Event::TerminalResponse(
                TerminalResponse::DeviceStatus(0)
            )))
        );
    }

    #[test]
    fn exposes_cursor_position_inside_a_response_transaction() {
        assert_eq!(
            parse_csi_cursor_position_with_capture(b"\x1b[20;10R", true).unwrap(),
            Some(InternalEvent::Event(Event::TerminalResponse(
                TerminalResponse::CursorPosition { column: 9, row: 19 }
            )))
        );
    }

    #[test]
    fn keeps_late_kitty_capability_replies_out_of_keyboard_input() {
        let response = b"\x1b_Gi=31;OK\x1b\\";
        for end in 2..response.len() {
            assert_eq!(
                parse_apc_with_capture(&response[..end], false, true).unwrap(),
                None,
                "partial Kitty reply of {end} bytes must remain buffered"
            );
        }
        assert_eq!(
            parse_apc_with_capture(response, false, true).unwrap(),
            Some(InternalEvent::Event(Event::TerminalResponse(
                TerminalResponse::KittyGraphics
            )))
        );
