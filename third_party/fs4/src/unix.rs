macro_rules! lock_impl {
    ($file: ty) => {
        ...
        fn flock(file: &$file, flag: rustix::fs::FlockOperation) -> std::io::Result<()> {
            ...
            match lock_backend(borrowed_fd, flag) {
                Ok(_) => Ok(()),
        ...
        #[cfg(all(not(target_os = "wasi"), any(target_os = "solaris", target_os = "illumos")))]
        fn lock_backend(
            fd: rustix::fd::BorrowedFd<'_>,
            flag: rustix::fs::FlockOperation,
        ) -> rustix::io::Result<()> {
            rustix::fs::fcntl_lock(fd, flag)
        }
