...
cfg_if! {
    if #[cfg(getrandom_backend = "custom")] {
        ...
        // uClibc does not consistently expose `getrandom` in Rust libc.
        // Resolve the symbol dynamically and preserve the crate's secure
        // /dev/urandom fallback if libc/kernel/seccomp does not provide it.
