...
cfg_if! {
    if #[cfg(getrandom_backend = "custom")] {
        ...
        // uClibc targets do not consistently expose `getrandom` through the
        // Rust libc crate. Resolve it dynamically when available and keep the
        // crate's cryptographically secure `/dev/urandom` fallback otherwise.
