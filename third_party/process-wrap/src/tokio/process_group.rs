...
impl ProcessGroupChild {
	...
	fn signal_imp(&self, sig: Signal) -> Result<()> {
		...
		{
			// Fuchsia libc implements POSIX process groups and `kill`, while nix
			// intentionally does not expose its `killpg` convenience wrapper. POSIX
			// defines killpg(pgid, sig) as kill(-pgid, sig), preserving group-wide
			// signal semantics rather than degrading to killing only the leader.
			let rc = unsafe { libc::kill(-self.pgid.as_raw(), sig as libc::c_int) };
			if rc == 0 {
				Ok(())
			} else {
		...
		{
			nix::sys::signal::killpg(self.pgid, sig).map_err(Error::from)
		}
	}
