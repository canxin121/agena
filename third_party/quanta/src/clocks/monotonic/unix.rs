...
impl Monotonic {
    ...
    pub fn now(self) -> u64 {
        // Some libc targets add private ABI padding to `timespec`, so a
        // public-field struct literal is not portable. `timespec` contains
        // only integer/padding storage and `clock_gettime` initializes the
        // public value fields before they are read.
