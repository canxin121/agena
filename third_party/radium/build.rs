...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // scripts. Prefer that authoritative list over the legacy target-triple
    // table below so newer 32-bit targets (for example riscv32gc) do not
    // accidentally name unavailable AtomicU64/AtomicI64 types.
    if let Ok(widths) = env::var("CARGO_CFG_TARGET_HAS_ATOMIC") {
        ...
        #[allow(clippy::match_single_binding, clippy::single_match)]
        match tgt_arch {
            "armv5te" | "mips" | "mipsel" | "powerpc" | "riscv32imac" | "thumbv7em"
