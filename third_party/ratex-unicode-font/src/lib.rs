...
// Libraries embedded in an alternate-screen application must not write to the
// controlling terminal behind the application's renderer. Keep upstream's
// useful discovery diagnostics available for explicit troubleshooting while
// making the normal library path silent.
macro_rules! font_diagnostic {
    ($($argument:tt)*) => {
        if std::env::var_os("RATEX_UNICODE_FONT_DIAGNOSTICS").is_some() {
            eprintln!($($argument)*);
        }
    };
}
...
fn discover_system_font() -> Option<(Arc<Vec<u8>>, u32)> {
    ...
    for &spec in candidates {
        if let Some(font) = load_font_spec(spec) {
            font_diagnostic!("[ratex-unicode-font] found via builtin path: {}", spec);
            return Some(font);
        }
    }
    ...
    font_diagnostic!("[ratex-unicode-font] no Unicode font found");
    None
}
